...
#include <fermat/io/zero_copy_stream_as_streambuf.h>
#include <fermat/io/binary_printer.h>
#include <turbo/log/logging.h>
#include <turbo/threading/thread_local.h>
#include <utility>
#include <cstdint>
// For CordBuffer::appendv(const const_iovec*, size_t). The only difference of this
...
namespace fermat {
    // CordBuffer is a non-continuous buffer that can be cut and combined w/o copying
    // payload. It can be read from or flushed into file descriptors as well.
    // CordBuffer is [thread-compatible]. Namely using different CordBuffer in different
    // threads simultaneously is safe, and reading a static CordBuffer from different
    // threads is safe as well.
    // CordBuffer is [NOT thread-safe]. Modifying a same CordBuffer from different threads
    ...
    public:
        ...
        // CordBuffer is essentially a tiny queue of BlockRefs.
        struct SmallView {
            BlockRef refs[2];
        };
        ...
        struct BigView {
           ...
           [[nodiscard]] const BlockRef &ref_at(uint32_t i) const { return refs[(start + i) & cap_mask]; }

           [[nodiscard]] BlockRef &ref_at(uint32_t i) { return refs[(start + i) & cap_mask]; }

           [[nodiscard]] uint32_t capacity() const { return cap_mask + 1; }
        };

        struct Movable {
            explicit Movable(CordBuffer &v) : _v(&v) {
            }

           [[nodiscard]] CordBuffer &value() const { return *_v; }
        ...
        CordBuffer(const CordBuffer &);

        CordBuffer(const Movable &);

        ~CordBuffer() { clear(); }

        CordBuffer& operator=(const CordBuffer &);

        CordBuffer& operator=(const Movable &);

        CordBuffer& operator=(std::string_view);

        // Exchange internal fields with another CordBuffer.
        void swap(CordBuffer &) noexcept;
        ...
        // Returns bytes cut.
        size_t cutn(CordBuffer *out, size_t n);
        ...
        // or other errors.
        int cut_until(CordBuffer *out, char const *delim);

        // std::string version, `delim' could be binary
        int cut_until(CordBuffer *out, const std::string &delim);
        ...
        static ssize_t cut_multiple_into_file_descriptor(
            int fd, CordBuffer *const*pieces, size_t count);
        ...
        static ssize_t cut_multiple_into_SSL_channel(
            struct ssl_st *ssl, CordBuffer *const*pieces, size_t count, int *ssl_error);

        // Append another CordBuffer to back side, payload of the CordBuffer is shared
        // rather than copied.
        void append(const CordBuffer &other);
        ...
        // IOBufBuilder instead, which reduce overhead by owning CordBuffer::Block.
        // ===================================================================
        ...
        int appendv(const fconst_iovec vec[], size_t n);

        int appendv(const iovec *vec, size_t n) { return appendv((const fconst_iovec *) vec, n); }
        ...
        // deleted using the deleter func when no CordBuffer references it anymore.
        ...
        // Get the data meta of the first byte in this CordBuffer.
        ...
        // Returns 0 on success, -1 otherwise.
        // [Rules]
        // 1. Make sure the CordBuffer to be assigned was NOT cut/pop from front side
        //    after reserving, otherwise behavior of this function is undefined,
        //    even if it returns 0.
        // 2. Make sure the CordBuffer to be assigned was NOT copied to/from another
        //    CordBuffer after reserving to prevent underlying blocks from being shared,
        //    otherwise the assignment affects all CordBuffer sharing the blocks, which
        ...
        size_t append_to(CordBuffer *buf, size_t n = (size_t) -1L, size_t pos = 0) const;
       ...
       [[nodiscard]] std::string to_string() const;
        ...
        //                      so user should not change this CordBuffer during using
        //                      the internal buffer.
        ...
        [[nodiscard]] const void *fetch1() const;
        ...
        // Equal with a string/CordBuffer or not.
       [[nodiscard]] bool equals(std::string_view) const;

       [[nodiscard]] bool equals(const CordBuffer &other) const;

        // Get the number of backing blocks
       [[nodiscard]] size_t backing_block_num() const { return _ref_num(); }

        // Get #i backing_block, an empty std::string_view is returned if no such block
       [[nodiscard]] std::string_view backing_block(size_t i) const;

        // Make a movable version of self
        Movable movable() { return Movable(*this); }

    protected:
        int _cut_by_char(CordBuffer *out, char);

        int _cut_by_delim(CordBuffer *out, char const *dbegin, size_t ndelim);

        // Returns: true iff this should be viewed as SmallView
       [[nodiscard]] bool _small() const;
      ...
      [[nodiscard]]  const BlockRef &_ref_at(size_t i) const;

        // Get pointer to n-th BlockRef(counting from front)
        // If i is out-of-range, nullptr is returned.
      [[nodiscard]]  const BlockRef *_pref_at(size_t i) const;
    ...
    std::ostream &operator<<(std::ostream &, const CordBuffer &buf);

    inline bool operator==(const CordBuffer &b, const std::string_view &s) { return b.equals(s); }
    inline bool operator==(const std::string_view &s, const CordBuffer &b) { return b.equals(s); }
    inline bool operator!=(const CordBuffer &b, const std::string_view &s) { return !b.equals(s); }
    inline bool operator!=(const std::string_view &s, const CordBuffer &b) { return !b.equals(s); }
    inline bool operator==(const CordBuffer &b1, const CordBuffer &b2) { return b1.equals(b2); }
    inline bool operator!=(const CordBuffer &b1, const CordBuffer &b2) { return !b1.equals(b2); }

    // IOPortal is a subclass of CordBuffer that can read from file descriptors.
    // Typically used as the buffer to store bytes from sockets.
    class IOPortal : public CordBuffer {
    public:
        IOPortal() : _block(nullptr) {
        }

        IOPortal(const IOPortal &rhs) : CordBuffer(rhs), _block(nullptr) {
        }
        ...
        // let continuing code on CordBuffer to reuse the blocks. Calling this function
    ...
    class IOReserveAlignedBuf : public CordBuffer {
    public:
    ...
    // Specialized utility to cut from CordBuffer faster than using corresponding
    // methods in CordBuffer.
    // Designed for efficiently parsing data from CordBuffer.
    // The cut CordBuffer can be appended during cutting.
    class IOBufCutter {
    public:
        explicit IOBufCutter(CordBuffer *buf);
        ...
        // Returns bytes copied.
        size_t copy_to(void *c, size_t n);
    ...
    private:
        void *_data;
        void *_data_end;
        CordBuffer::Block *_block;
        CordBuffer *_buf;
    };

    // Parse protobuf message from CordBuffer. Notice that this wrapper does not change
    // source CordBuffer, which also should not change during lifetime of the wrapper.
    // Even if a IOBufAsZeroCopyInputStream is created but parsed, the source
    // CordBuffer should not be changed as well becuase constructor of the stream
    // saves internal information of the source CordBuffer which is assumed to be
    // unchanged.
    // Example:
    ...
    public:
        explicit IOBufAsZeroCopyInputStream(const CordBuffer &);
    ...
    private:
        ...
        int _add_offset;
        int64_t _byte_count;
        const CordBuffer *_buf;
    };

    // Serialize protobuf message into CordBuffer. This wrapper does not clear source
    // CordBuffer before appending. You can change the source CordBuffer when stream is 
    // not used(append sth. to the CordBuffer, serialize a protobuf message, append 
    // sth. again, serialize messages again...). This is different from 
    // IOBufAsZeroCopyInputStream which needs the source CordBuffer to be unchanged.
    // Example:
    ...
    public:
        explicit IOBufAsZeroCopyOutputStream(CordBuffer *);

        IOBufAsZeroCopyOutputStream(CordBuffer *, uint32_t block_size);
    ...
    private:
        ...
        CordBuffer::Block *_cur_block;
        int64_t _byte_count;
    };


    // A std::ostream to build CordBuffer.
    // Example:
    //   IOBufBuilder builder;
    //   builder << "Anything that can be sent to std::ostream";
    //   // You have several methods to fetch the CordBuffer.
    ...
    public:
        ...
        void buf(const CordBuffer &buf) {
            *static_cast<CordBuffer *>(this) = buf;
        }

        void move_to(CordBuffer &target) {
            target = Movable(buf());
        }
    };

    // Create CordBuffer by appending data *faster*
    class IOBufAppender {
    public:
        IOBufAppender();

        // Append `n' bytes starting from `data' to back side of the internal buffer
        // Costs 2/3 time of CordBuffer.append for short data/strings on Intel(R) Xeon(R)
        // CPU E5-2620 @ 2.00GHz. Longer data/strings make differences smaller.
        // Returns 0 on success, -1 otherwise.
      [[nodiscard]]  int append(const void *data, size_t n);

      [[nodiscard]]  int append(std::string_view str) {
        ...
        // Costs ~3ns while CordBuffer.push_back costs ~13ns on Intel(R) Xeon(R) CPU
        // E5-2620 @ 2.00GHz
        // Returns 0 on success, -1 otherwise.
        int push_back(char c);
    ...
    // Iterate bytes of a CordBuffer.
    // During iteration, the iobuf should NOT be changed.
    class IOBufBytesIterator {
    public:
        explicit IOBufBytesIterator(const CordBuffer &buf);
        ...
        size_t append_and_forward(CordBuffer *buf, size_t n);

        bool forward_one_block(const void **data, size_t *size);

       [[nodiscard]] size_t bytes_left() const { return _bytes_left; }
    ...
    inline ssize_t CordBuffer::cut_into_file_descriptor(int fd, size_t size_hint) {
        return pcut_into_file_descriptor(fd, -1, size_hint);
    }

    inline ssize_t CordBuffer::cut_multiple_into_file_descriptor(
        int fd, CordBuffer *const*pieces, size_t count) {
        return pcut_multiple_into_file_descriptor(fd, -1, pieces, count);
    }

    inline int CordBuffer::append_user_data(void *data, size_t size, std::function<void(void *)> deleter) {
    ...
    inline void reset_block_ref(CordBuffer::BlockRef &ref) {
        ref.offset = 0;
        ref.length = 0;
        ref.block = nullptr;
    }
    ...
    inline CordBuffer::CordBuffer(const Movable &rhs) {
        _sv = rhs.value()._sv;
        new(&rhs.value()) CordBuffer;
    }

    inline CordBuffer& CordBuffer::operator=(const Movable &rhs) {
        clear();
        _sv = rhs.value()._sv;
        new(&rhs.value()) CordBuffer;
        return *this;
    ...
    inline CordBuffer& CordBuffer::operator=(std::string_view str) {
        clear();
        append(str);
        return *this;
    }

    inline void CordBuffer::swap(CordBuffer &other) noexcept {
        const SmallView tmp = other._sv;
        other._sv = _sv;
        _sv = tmp;
    }

    inline int CordBuffer::cut_until(CordBuffer *out, char const *delim) {
        if (*delim) {
            if (!*(delim + 1)) {
    ...
    inline int CordBuffer::cut_until(CordBuffer *out, const std::string &delim) {
        if (delim.length() == 1UL) {
    ...
    inline int CordBuffer::append(std::string_view s) {
        return append(s.data(), s.length());
    }

    inline std::string CordBuffer::to_string() const {
        std::string s;
        copy_to(&s);
        return s;
    }

    inline bool CordBuffer::empty() const {
    ...
    inline bool CordBuffer::_small() const {
        return _bv.magic >= 0;
    }

    inline size_t CordBuffer::_ref_num() const {
        return _small()
                   ? (!!_sv.refs[0].block + !!_sv.refs[1].block)
                   : _bv.nref;
    }

    inline CordBuffer::BlockRef &CordBuffer::_front_ref() {
        return _small() ? _sv.refs[0] : _bv.refs[_bv.start];
    }

    inline const CordBuffer::BlockRef &CordBuffer::_front_ref() const {
        return _small() ? _sv.refs[0] : _bv.refs[_bv.start];
    }

    inline CordBuffer::BlockRef &CordBuffer::_back_ref() {
        return _small() ? _sv.refs[!!_sv.refs[1].block] : _bv.ref_at(_bv.nref - 1);
    }

    inline const CordBuffer::BlockRef &CordBuffer::_back_ref() const {
        return _small() ? _sv.refs[!!_sv.refs[1].block] : _bv.ref_at(_bv.nref - 1);
    }

    inline CordBuffer::BlockRef &CordBuffer::_ref_at(size_t i) {
        return _small() ? _sv.refs[i] : _bv.ref_at(i);
    }

    inline const CordBuffer::BlockRef &CordBuffer::_ref_at(size_t i) const {
        return _small() ? _sv.refs[i] : _bv.ref_at(i);
    }

    inline const CordBuffer::BlockRef *CordBuffer::_pref_at(size_t i) const {
        if (_small()) {
    ...
    inline bool operator==(const CordBuffer::BlockRef &r1, const CordBuffer::BlockRef &r2) {
    ...
    inline void CordBuffer::_push_back_ref(const BlockRef &r) {
        if (_small()) {
    ...
    inline void CordBuffer::_move_back_ref(const BlockRef &r) {
        if (_small()) {
    ...
    /////////////// IOBufAppender /////////////////
    inline int IOBufAppender::append(const void *data, size_t n) {
        do {
            const size_t size = (char *) _data_end - (char *) _data;
            if (n <= size) {
                memcpy(_data, data, n);
    ...
    inline IOBufBytesIterator::IOBufBytesIterator(const CordBuffer &buf)
    ...
    struct CordBuffer::Block {
...
#ifndef NDEBUG
            if (abi_check != 0) {
                KLOG(FATAL) << "Your program seems to wrongly contain two "
                        "ABI-incompatible implementations of CordBuffer";
            }
#endif
        }
      ...
      [[nodiscard]]  bool full() const { return size >= cap; }
       [[nodiscard]] size_t left_space() const { return cap - size; }

    private:
      [[nodiscard]]  bool is_samplable() const {
    ...
    namespace iobuf {
        ...
        inline void release_tls_block(CordBuffer::Block *b) {
            if (!b) {
                return;
            }
            ...
            if (b->full()) {
                ...
                b->dec_ref();
                // g_num_hit_tls_threshold.fetch_add(1, memory_order_relaxed);
        ...
        inline CordBuffer::Block *create_block(const size_t block_size) {
            ...
            return new(mem) CordBuffer::Block(mem + sizeof(CordBuffer::Block),
                                              block_size - sizeof(CordBuffer::Block));
        }

        inline CordBuffer::Block *create_block() {
            return create_block(CordBuffer::DEFAULT_BLOCK_SIZE);
        }
...
namespace std {
    template<>
    inline void swap(fermat::CordBuffer &a, fermat::CordBuffer &b) {
        return a.swap(b);
    }
} // namespace std
