...
namespace fermat::uri::helpers {
    ...
    [[nodiscard]] inline std::optional<std::string_view> prune_hash(
    ...
    [[nodiscard]] inline bool shorten_path(fermat::KString &path,
    ...
    /// @see https://url.spec.whatwg.org/#shorten-a-urls-path
    /// @returns Returns true if path is shortened.
    [[nodiscard]] inline bool shorten_path(std::string_view &path,
                                           fermat::uri::scheme::type type) noexcept;
    ...
    [[nodiscard]] inline void parse_prepared_path(std::string_view input,
                                                  fermat::uri::scheme::type type,
                                                  fermat::KString &path);

    /// @private
    /// Remove and mutate all ASCII tab or newline characters from an input.
    [[nodiscard]] inline void remove_ascii_tab_or_newline(fermat::KString &input) noexcept;

    /// @private
    /// Return the substring from input going from index pos to the end.
    /// This function cannot throw.
    [[nodiscard]] inline std::string_view substring(std::string_view input,
                                                    size_t pos) noexcept;

    /// @private
    /// Returns true if the string_view points within the string.
    [[nodiscard]] bool overlaps(std::string_view input1, const fermat::KString &input2) noexcept;

    /// @private
    /// Return the substring from input going from index pos1 to the pos2 (non
    /// included). The length of the substring is pos2 - pos1.
    [[nodiscard]] inline std::string_view substring(const fermat::KString &input,
                                                    size_t pos1,
    ...
    [[nodiscard]] inline void resize(std::string_view &input, size_t pos) noexcept;
    ...
    [[nodiscard]] inline std::pair<size_t, bool> get_host_delimiter_location(
    ...
    /// string.
    [[nodiscard]] inline void trim_c0_whitespace(std::string_view &input) noexcept;

    /// @private
    /// @see
    /// https://url.spec.whatwg.org/#potentially-strip-trailing-spaces-from-an-opaque-path
    template<class url_type>
    [[nodiscard]] inline void strip_trailing_spaces_from_opaque_path(
        url_type &url) noexcept;

    /// @private
    /// Finds the delimiter of a view in authority state.
    [[nodiscard]] inline size_t find_authority_delimiter_special(std::string_view view) noexcept;

    /// @private
    /// Finds the delimiter of a view in authority state.
    [[nodiscard]] inline size_t find_authority_delimiter(std::string_view view) noexcept;

    /// @private
    template<typename T, typename... Args>
    inline void inner_concat(fermat::KString &buffer, T t) {
        buffer.append(t);
    }

    /// @private
    template<typename T, typename... Args>
    inline void inner_concat(fermat::KString &buffer, T t, Args... args) {
        buffer.append(t);
    ...
    fermat::KString concat(Args... args) {
    ...
    inline int fast_digit_count(uint32_t x) noexcept {
        auto int_log2 = [](uint32_t z) -> int {
            return 31 - fermat::uri::helpers::leading_zeroes(z | 1);
        };
