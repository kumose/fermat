...
fermat::uri::result<fermat::uri::UrlAggregator> &get_instance(void *result) noexcept {
    return *(fermat::uri::result<fermat::uri::UrlAggregator> *) result;
}
...
ada_url ada_parse(const char *input, size_t length) noexcept {
    return new fermat::uri::result<fermat::uri::UrlAggregator>(
        fermat::uri::parse<fermat::uri::UrlAggregator>(std::string_view(input, length)));
}

ada_url ada_parse_with_base(const char *input, size_t input_length,
                            const char *base, size_t base_length) noexcept {
    auto base_out =
            fermat::uri::parse<fermat::uri::UrlAggregator>(std::string_view(base, base_length));

    if (!base_out) {
        return new fermat::uri::result<fermat::uri::UrlAggregator>(base_out);
    }

    return new fermat::uri::result<fermat::uri::UrlAggregator>(fermat::uri::parse<fermat::uri::UrlAggregator>(
        std::string_view(input, input_length), &base_out.value()));
}

bool ada_can_parse(const char *input, size_t length) noexcept {
    return fermat::uri::can_parse(std::string_view(input, length));
}

bool ada_can_parse_with_base(const char *input, size_t input_length,
                             const char *base, size_t base_length) noexcept {
    std::string_view base_view(base, base_length);
    return fermat::uri::can_parse(std::string_view(input, input_length), &base_view);
}

void ada_free(ada_url result) noexcept {
    auto *r = (fermat::uri::result<fermat::uri::UrlAggregator> *) result;
    delete r;
}

ada_url ada_copy(ada_url input) noexcept {
    fermat::uri::result<fermat::uri::UrlAggregator> &r = get_instance(input);
    return new fermat::uri::result<fermat::uri::UrlAggregator>(r);
}

bool ada_is_valid(ada_url result) noexcept {
    fermat::uri::result<fermat::uri::UrlAggregator> &r = get_instance(result);
    return r.has_value();
}
...
ada_owned_string ada_get_origin(ada_url result) noexcept {
    ...
    fermat::KString out = r->get_origin();
...
// returns a pointer to the internal UrlAggregator::UrlComponents
const ada_url_components *ada_get_components(ada_url result) noexcept {
    static_assert(sizeof(ada_url_components) == sizeof(fermat::uri::UrlComponents));
    fermat::uri::result<fermat::uri::UrlAggregator> &r = get_instance(result);
    if (!r) {
        return nullptr;
    }
...
ada_owned_string ada_idna_to_unicode(const char *input, size_t length) {
    fermat::KString out = fermat::uri::idna::to_unicode(std::string_view(input, length));
    ada_owned_string owned{};
...
ada_owned_string ada_idna_to_ascii(const char *input, size_t length) {
    fermat::KString out = fermat::uri::idna::to_ascii(std::string_view(input, length));
    ada_owned_string owned{};
    owned.length = out.size();
    owned.data = new char[owned.length];
    memcpy((void *) owned.data, out.data(), owned.length);
...
ada_url_search_params ada_parse_search_params(const char *input,
                                              size_t length) {
    return new fermat::uri::result<fermat::uri::UrlSearchParams>(
        fermat::uri::UrlSearchParams(std::string_view(input, length)));
}

void ada_free_search_params(ada_url_search_params result) {
    auto *r = (fermat::uri::result<fermat::uri::UrlSearchParams> *) result;
    delete r;
}

ada_owned_string ada_search_params_to_string(ada_url_search_params result) {
    fermat::uri::result<fermat::uri::UrlSearchParams> &r =
            *(fermat::uri::result<fermat::uri::UrlSearchParams> *) result;
    if (!r) return ada_owned_string{nullptr, 0};
    fermat::KString out = r->to_string();
    ada_owned_string owned{};
    owned.length = out.size();
    owned.data = new char[owned.length];
    memcpy((void *) owned.data, out.data(), owned.length);
...
ada_strings ada_search_params_get_all(ada_url_search_params result,
    ...
    fermat::uri::result<fermat::uri::UrlSearchParams> &r =
            *(fermat::uri::result<fermat::uri::UrlSearchParams> *) result;
    if (!r) {
        return new fermat::uri::result<fermat::Vector<fermat::KString> >(
            fermat::Vector<fermat::KString>());
    }
    return new fermat::uri::result<fermat::Vector<fermat::KString> >(
...
ada_url_search_params_keys_iter ada_search_params_get_keys(
    ...
    fermat::uri::result<fermat::uri::UrlSearchParams> &r =
            *(fermat::uri::result<fermat::uri::UrlSearchParams> *) result;
    if (!r) {
        return new fermat::uri::result<fermat::uri::UrlSearchParamsKeysIter>(
            fermat::uri::UrlSearchParamsKeysIter());
    }
    return new fermat::uri::result<fermat::uri::UrlSearchParamsKeysIter>(r->get_keys());
}
...
ada_url_search_params_values_iter ada_search_params_get_values(
    ...
    fermat::uri::result<fermat::uri::UrlSearchParams> &r =
            *(fermat::uri::result<fermat::uri::UrlSearchParams> *) result;
    if (!r) {
        return new fermat::uri::result<fermat::uri::url_search_params_values_iter>(
            fermat::uri::url_search_params_values_iter());
    }
    return new fermat::uri::result<fermat::uri::url_search_params_values_iter>(r->get_values());
}
...
ada_url_search_params_entries_iter ada_search_params_get_entries(
    ...
    fermat::uri::result<fermat::uri::UrlSearchParams> &r =
            *(fermat::uri::result<fermat::uri::UrlSearchParams> *) result;
    if (!r) {
        return new fermat::uri::result<fermat::uri::url_search_params_entries_iter>(
            fermat::uri::url_search_params_entries_iter());
    }
    return new fermat::uri::result<fermat::uri::url_search_params_entries_iter>(r->get_entries());
}

void ada_free_strings(ada_strings result) {
    auto *r = (fermat::uri::result<fermat::Vector<fermat::KString> > *) result;
    delete r;
}
...
void ada_free_search_params_keys_iter(ada_url_search_params_keys_iter result) {
    auto *r = (fermat::uri::result<fermat::uri::UrlSearchParamsKeysIter> *) result;
    delete r;
}
...
void ada_free_search_params_values_iter(
    ...
    auto *r = (fermat::uri::result<fermat::uri::url_search_params_values_iter> *) result;
    delete r;
}
...
void ada_free_search_params_entries_iter(
    ...
    auto *r = (fermat::uri::result<fermat::uri::url_search_params_entries_iter> *) result;
    delete r;
}
