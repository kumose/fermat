...
// Identify the compiler and declare the KM_COMPILER_xxxx defines
#include <fermat/base/config/eacompiler.h>

// Identify traits which this compiler supports, or does not support
#include <fermat/base/config/eacompilertraits.h>

// Identify the platform and declare the KM_xxxx defines
...
#if defined(KM_COMPILER_GNUC) || defined(KM_COMPILER_MSVC) || defined(KM_WCHAR_T_NON_NATIVE) || defined(KM_PLATFORM_SONY)
...
// is defined.
#if defined(KM_COMPILER_CLANG) && defined(KM_PLATFORM_APPLE)
	#include <cstddef>
#endif
...
// would have put this into the compilertraits header.
#if !defined(KM_COMPILER_HAS_INTTYPES) && (!defined(_MSC_VER) || (_MSC_VER > 1500)) && (defined(KM_COMPILER_IS_C99) || defined(INT8_MIN) || defined(KM_COMPILER_HAS_C99_TYPES) || defined(_SN_STDINT_H))
	#define KM_COMPILER_HAS_INTTYPES
#endif

#ifdef KM_COMPILER_HAS_INTTYPES // If the compiler supports inttypes...
	// ------------------------------------------------------------------------
	...
	#if !defined(KM_COMPILER_MSVC) || (defined(KM_COMPILER_MSVC) && KM_COMPILER_VERSION >= 1800)
	...
	#if defined(__MINGW32__) || (defined(KM_PLATFORM_ANDROID) && !(defined(KM_ANDROID_SDK_LEVEL) && KM_ANDROID_SDK_LEVEL >= 21))
...
// type sizes
...
#if defined(KM_COMPILER_HAS_INTTYPES) && (!defined(KM_COMPILER_MSVC) || (defined(KM_COMPILER_MSVC) && KM_COMPILER_VERSION >= 1800))
	#define KM_COMPILER_HAS_C99_FORMAT_MACROS
#endif

#ifndef KM_COMPILER_HAS_C99_FORMAT_MACROS
	// ------------------------------------------------------------------------
	...
	#if defined(KM_COMPILER_MSVC) // VC++ 7.1+ understands long long as a data type but doesn't accept %ll as a printf specifier.
		#define KM_PRI_64_LENGTH_SPECIFIER "I64"
		#define KM_SCN_64_LENGTH_SPECIFIER "I64"
	#else
		#define KM_PRI_64_LENGTH_SPECIFIER "ll"
		#define KM_SCN_64_LENGTH_SPECIFIER "ll"
	...
	#if defined(KM_COMPILER_IS_C99) || defined(KM_COMPILER_GNUC)
		#define PRId8     "hhd"
		#define PRIi8     "hhi"
		#define PRIo8     "hho"
		#define PRIu8     "hhu"
	...
	#define PRId64        KM_PRI_64_LENGTH_SPECIFIER "d"
	#define PRIi64        KM_PRI_64_LENGTH_SPECIFIER "i"
	#define PRIo64        KM_PRI_64_LENGTH_SPECIFIER "o"
	#define PRIu64        KM_PRI_64_LENGTH_SPECIFIER "u"
	#define PRIx64        KM_PRI_64_LENGTH_SPECIFIER "x"
	#define PRIX64        KM_PRI_64_LENGTH_SPECIFIER "X"
	...
	#define SCNd64        KM_SCN_64_LENGTH_SPECIFIER "d"
	#define SCNi64        KM_SCN_64_LENGTH_SPECIFIER "i"
	#define SCNo64        KM_SCN_64_LENGTH_SPECIFIER "o"
	#define SCNu64        KM_SCN_64_LENGTH_SPECIFIER "u"
	#define SCNx64        KM_SCN_64_LENGTH_SPECIFIER "x"

	#if defined(KM_COMPILER_MSVC) && (KM_COMPILER_VERSION >= 1900)
		#define SCNdPTR       PRIdPTR
		#define SCNiPTR       PRIiPTR
		#define SCNoPTR       PRIoPTR
		#define SCNuPTR       PRIuPTR
		#define SCNxPTR       PRIxPTR
	#elif (KM_PLATFORM_PTR_SIZE == 4)
...
// 
	...
	#if defined(KM_COMPILER_MSVC) || (defined(KM_COMPILER_INTEL) && defined(KM_PLATFORM_WINDOWS))
		#if defined(__cplusplus)
			typedef bool bool8_t;
		#else
			typedef int8_t bool8_t;
		#endif
	#else // KM_COMPILER_GNUC generally uses 4 bytes per bool.
		typedef int8_t bool8_t;
	#endif
#endif
...
//
#if !defined(_INTPTR_T_DEFINED) && !defined(_intptr_t_defined) && !defined(KM_COMPILER_HAS_C99_TYPES)
	#if (KM_PLATFORM_PTR_SIZE == 4)
...
#if !defined(_UINTPTR_T_DEFINED) && !defined(_uintptr_t_defined) && !defined(KM_COMPILER_HAS_C99_TYPES)
	#if (KM_PLATFORM_PTR_SIZE == 4)
...
#if !defined(KM_COMPILER_HAS_INTTYPES)
...
//
	...
	// so for all compilers, as size_t may be based on int, long, or long long.
		...
		#if defined(_MSC_VER) && (KM_PLATFORM_PTR_SIZE == 8)
...
//
#if defined(KM_COMPILER_MSVC)
	#if defined(KM_WCHAR_T_NON_NATIVE)
...
// _CHAR16T and let EABase define char16_t and KM_CHAR16.
//
...
//
// The KM_WCHAR_UNIQUE symbol is defined to 1 if wchar_t is distinct from
...
//
// The KM_CHAR8_UNIQUE symbol is defined to 1 if char8_t is distinct type
// from char in the type system, and defined to 0 if otherwise.

#if !defined(KM_CHAR16_NATIVE)
	// To do: Change this to be based on KM_COMPILER_NO_NEW_CHARACTER_TYPES.
	#if defined(_MSC_VER) && (_MSC_VER >= 1600) && defined(_HAS_CHAR16_T_LANGUAGE_SUPPORT) && _HAS_CHAR16_T_LANGUAGE_SUPPORT // VS2010+
		#define KM_CHAR16_NATIVE 1
	#elif defined(KM_COMPILER_CLANG) && defined(KM_COMPILER_CPP11_ENABLED)
		#if __has_feature(cxx_unicode_literals)
			#define KM_CHAR16_NATIVE 1
		#elif (KM_COMPILER_VERSION >= 300) && !(defined(KM_PLATFORM_IPHONE) || defined(KM_PLATFORM_OSX))
			#define KM_CHAR16_NATIVE 1
		#elif defined(KM_PLATFORM_APPLE)
			#define KM_CHAR16_NATIVE 1
		#else
			#define KM_CHAR16_NATIVE 0
		#endif
	#elif defined(__EDG_VERSION__) && (__EDG_VERSION__ >= 404) && defined(__CHAR16_TYPE__) && defined(KM_COMPILER_CPP11_ENABLED)// EDG 4.4+.
		#define KM_CHAR16_NATIVE 1
	#elif defined(KM_COMPILER_GNUC) && (KM_COMPILER_VERSION >= 4004) && !defined(KM_COMPILER_EDG) && (defined(KM_COMPILER_CPP11_ENABLED) || defined(__STDC_VERSION__)) // g++ (C++ compiler) 4.4+ with -std=c++0x or gcc (C compiler) 4.4+ with -std=gnu99
		#define KM_CHAR16_NATIVE 1
	#else
		#define KM_CHAR16_NATIVE 0
	#endif
...
#if !defined(KM_CHAR32_NATIVE)                    // Microsoft currently ties char32_t language support to char16_t language support. So we use CHAR16_T here.
	// To do: Change this to be based on KM_COMPILER_NO_NEW_CHARACTER_TYPES.
	#if defined(_MSC_VER) && (_MSC_VER >= 1600) && defined(_HAS_CHAR16_T_LANGUAGE_SUPPORT) && _HAS_CHAR16_T_LANGUAGE_SUPPORT // VS2010+
		#define KM_CHAR32_NATIVE 1
	#elif defined(KM_COMPILER_CLANG) && defined(KM_COMPILER_CPP11_ENABLED)
		#if __has_feature(cxx_unicode_literals)
			#define KM_CHAR32_NATIVE 1
		#elif (KM_COMPILER_VERSION >= 300) && !(defined(KM_PLATFORM_IPHONE) || defined(KM_PLATFORM_OSX))
			...
			#define KM_CHAR32_NATIVE 1
		#else
			#define KM_CHAR32_NATIVE 0
		#endif
	#elif defined(__EDG_VERSION__) && (__EDG_VERSION__ >= 404) && defined(__CHAR32_TYPE__) && defined(KM_COMPILER_CPP11_ENABLED)// EDG 4.4+.
		#define KM_CHAR32_NATIVE 1
	#elif defined(KM_COMPILER_GNUC) && (KM_COMPILER_VERSION >= 4004) && !defined(KM_COMPILER_EDG) && (defined(KM_COMPILER_CPP11_ENABLED) || defined(__STDC_VERSION__)) // g++ (C++ compiler) 4.4+ with -std=c++0x or gcc (C compiler) 4.4+ with -std=gnu99
		#define KM_CHAR32_NATIVE 1
	#else
...
#if KM_CHAR16_NATIVE || KM_CHAR32_NATIVE
	#define KM_WCHAR_UNIQUE 1
#else
	#define KM_WCHAR_UNIQUE 0
#endif
...
//
// KM_CHAR8_UNIQUE is 1 when the type is a unique in the type system and
// can there be used as a valid overload. KM_CHAR8_UNIQUE is 0 otherwise.
//
// http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2018/p0482r6.html
//
#ifdef __cpp_char8_t
	#define CHAR8_T_DEFINED
	#define KM_CHAR8_UNIQUE 1
#else
	#define KM_CHAR8_UNIQUE 0
#endif
...
//
#define KM_LIMITS_DIGITS_S(T)  ((sizeof(T) * 8) - 1)
#define KM_LIMITS_DIGITS_U(T)  ((sizeof(T) * 8))
#define KM_LIMITS_DIGITS(T)    ((KM_LIMITS_IS_SIGNED(T) ? KM_LIMITS_DIGITS_S(T) : KM_LIMITS_DIGITS_U(T)))
#define KM_LIMITS_IS_SIGNED(T) ((T)(-1) < 0)
#define KM_LIMITS_MIN_S(T)     ((T)((T)1 << KM_LIMITS_DIGITS_S(T)))
#define KM_LIMITS_MIN_U(T)     ((T)0)
#define KM_LIMITS_MIN(T)       ((KM_LIMITS_IS_SIGNED(T) ? KM_LIMITS_MIN_S(T) : KM_LIMITS_MIN_U(T)))
#define KM_LIMITS_MAX_S(T)     ((T)(((((T)1 << (KM_LIMITS_DIGITS(T) - 1)) - 1) << 1) + 1))
#define KM_LIMITS_MAX_U(T)     ((T)~(T)0)
#define KM_LIMITS_MAX(T)       ((KM_LIMITS_IS_SIGNED(T) ? KM_LIMITS_MAX_S(T) : KM_LIMITS_MAX_U(T)))

#if !defined(CHAR8_MIN)
	#define CHAR8_MIN KM_LIMITS_MIN(char8_t)
#endif

#if !defined(CHAR8_MAX)
	#define CHAR8_MAX KM_LIMITS_MAX(char8_t)
#endif

#if !defined(CHAR16_MIN)
	#define CHAR16_MIN KM_LIMITS_MIN(char16_t)
#endif

#if !defined(CHAR16_MAX)
	#define CHAR16_MAX KM_LIMITS_MAX(char16_t)
#endif

#if !defined(CHAR32_MIN)
	#define CHAR32_MIN KM_LIMITS_MIN(char32_t)
#endif

#if !defined(CHAR32_MAX)
	#define CHAR32_MAX KM_LIMITS_MAX(char32_t)
#endif



// KM_CHAR8 / KM_CHAR16 / KM_CHAR32 / KM_WCHAR
//
// Supports usage of portable string constants.
//
// Example usage:
//     const char16_t* str = KM_CHAR16("Hello world");
//     const char32_t* str = KM_CHAR32("Hello world");
//     const char16_t  c   = KM_CHAR16('\x3001');
//     const char32_t  c   = KM_CHAR32('\x3001');
//
#ifndef KM_CHAR8
	#if KM_CHAR8_UNIQUE
		#define KM_CHAR8(s) u8 ## s
	#else
		#define KM_CHAR8(s) s
	#endif
#endif
...
#ifndef KM_WCHAR
	#define KM_WCHAR_(s) L ## s
	#define KM_WCHAR(s)  KM_WCHAR_(s)
#endif

#ifndef KM_CHAR16
	#if KM_CHAR16_NATIVE && !defined(_MSC_VER) // Microsoft doesn't support char16_t string literals.
		#define KM_CHAR16_(s) u ## s
		#define KM_CHAR16(s)  KM_CHAR16_(s)
	#elif (KM_WCHAR_SIZE == 2)
		#if defined(_MSC_VER) && (_MSC_VER >= 1900) && defined(__cplusplus) // VS2015 supports u"" string literals.
			#define KM_CHAR16_(s) u ## s
			#define KM_CHAR16(s)  KM_CHAR16_(s)
		#else
			#define KM_CHAR16_(s) L ## s
			#define KM_CHAR16(s)  KM_CHAR16_(s)
		#endif
	#else
		//#define KM_CHAR16(s) // Impossible to implement efficiently.
	#endif
#endif

#ifndef KM_CHAR32
	#if KM_CHAR32_NATIVE && !defined(_MSC_VER) // Microsoft doesn't support char32_t string literals.
		#define KM_CHAR32_(s) U ## s
		#define KM_CHAR32(s)  KM_CHAR32_(s)
	#elif (KM_WCHAR_SIZE == 2)
		#if defined(_MSC_VER) && (_MSC_VER >= 1900) && defined(__cplusplus) // VS2015 supports u"" string literals.
			#define KM_CHAR32_(s) U ## s
			#define KM_CHAR32(s)  KM_CHAR32_(s)
		#else
			//#define KM_CHAR32(s) // Impossible to implement.
		#endif
	#elif (KM_WCHAR_SIZE == 4)
		#define KM_CHAR32_(s) L ## s
		#define KM_CHAR32(s)  KM_CHAR32_(s)
	#else
...
//
	...
	#define EAText16(x)  KM_CHAR16(x)
	#define EAChar16(x)  KM_CHAR16(x)
#endif
...
//
...
#elif !defined(__cplusplus) && defined(KM_PLATFORM_ANDROID) && ((defined(__STDC_VERSION__) && __STDC_VERSION__ < 201100L) || !defined(__STDC_VERSION__))
...
#elif defined(__EDG_VERSION__) && (__EDG_VERSION__ >= 401) && defined(KM_COMPILER_CPP11_ENABLED)
	// static_assert is defined by the compiler.
	...
	#define NEED_CUSTOM_STATIC_ASSERT
		...
		#define KM_STATIC_ASSERT_UNUSED_ATTRIBUTE        __attribute__((unused))
	#else
		#define KM_STATIC_ASSERT_UNUSED_ATTRIBUTE
	#endif
	#define KM_STATIC_ASSERT_TOKEN_PASTE(a,b)        a ## b
	#define KM_STATIC_ASSERT_CONCATENATE_HELPER(a,b) KM_STATIC_ASSERT_TOKEN_PASTE(a,b)

	#if defined(__COUNTER__) // If this extension is available, which allows multiple statements per line...
		#define static_assert(expression, description) typedef char KM_STATIC_ASSERT_CONCATENATE_HELPER(compileTimeAssert,__COUNTER__) [((expression) != 0) ? 1 : -1] KM_STATIC_ASSERT_UNUSED_ATTRIBUTE
	#else
		#define static_assert(expression, description) typedef char KM_STATIC_ASSERT_CONCATENATE_HELPER(compileTimeAssert,__LINE__) [((expression) != 0) ? 1 : -1] KM_STATIC_ASSERT_UNUSED_ATTRIBUTE
	#endif
...
// KM_IS_ENABLED
//
// KM_IS_ENABLED is intended to be used for detecting if compile time features are enabled or disabled.
//
...
// must be set to either KM_ENABLED or KM_DISABLED.  (Do not try to set the feature define directly to some other
// value.)
//
...
//      // KM_ENABLED or KM_DISABLED.
//      #define USER_PROVIDED_FEATURE_DEFINE KM_ENABLED
//
//      #if KM_IS_ENABLED(USER_PROVIDED_FEATURE_DEFINE)
//          // USER_PROVIDED_FEATURE_DEFINE is enabled
//      #else
//          // USER_PROVIDED_FEATURE_DEFINE is disabled
//      #endif
//
#define KM_ENABLED              111-
#define KM_DISABLED             333-
// NOTE: Numeric values for x will produce a parse error while empty values produce a divide by zero, and the test is a bool for proper std::negation behavior
#define KM_IS_ENABLED(x) (333 == 333 * 111 / ((x 0) * (((x 0) == 333 ? 1 : 0) + ((x 0) == 111 ? 1 : 0))))
...
#ifdef __cplusplus
	#include <fermat/base/int128.h>
#endif


#if !defined(KM_GET_INSTRUCTION_POINTER)
	#if defined(KM_COMPILER_CLANG) || defined(KM_COMPILER_GNUC)
		#define KM_GET_INSTRUCTION_POINTER() []()KM_NO_INLINE{ return __builtin_return_address(0); }()
	#elif defined(KM_COMPILER_MSVC) && !defined(_MANAGED)
		#include <intrin.h>
		#define KM_GET_INSTRUCTION_POINTER() []()KM_NO_INLINE{ return _ReturnAddress(); }()
	#else
		#define KM_GET_INSTRUCTION_POINTER() nullptr
	#endif
#endif
