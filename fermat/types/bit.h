...
#if defined(KM_COMPILER_MSVC) && !defined(__clang__)
#include <intrin.h>
#endif
...
namespace fermat {
...
#if defined(FERMAT_CONSTEXPR_BIT_CAST_SUPPORTED) && FERMAT_CONSTEXPR_BIT_CAST_SUPPORTED
	...
	inline To bit_cast(const From &from) noexcept {
		typename fermat::aligned_storage<sizeof(To), alignof(To)>::type to;
		::memcpy(fermat::addressof(to), fermat::addressof(from), sizeof(To));
		return reinterpret_cast<To &>(to);
	}

#endif // FERMAT_CONSTEXPR_BIT_CAST_SUPPORTED
	...
	namespace internal {
		...
		inline int countl_zero(unsigned char x) noexcept {
			unsigned long index;
			return _BitScanReverse(&index, static_cast<unsigned long>(x))
				       ? (sizeof(unsigned char) * CHAR_BIT - 1 - index)
		...
		inline int countl_zero(unsigned short x) noexcept {
			unsigned long index;
			return _BitScanReverse(&index, static_cast<unsigned long>(x))
				       ? (sizeof(unsigned short) * CHAR_BIT - 1 - index)
		...
		inline int countl_zero(unsigned int x) noexcept {
			unsigned long index;
			return _BitScanReverse(&index, static_cast<unsigned long>(x))
				       ? (sizeof(unsigned int) * CHAR_BIT - 1 - index)
		...
		inline int countl_zero(unsigned long x) noexcept {
			...
			return _BitScanReverse(&index, x)
				       ? (sizeof(unsigned long) * CHAR_BIT - 1 - index)
				       : (sizeof(unsigned long) * CHAR_BIT);
		}

#if (KM_PLATFORM_PTR_SIZE == 8)
		inline int countl_zero(unsigned long long x) noexcept {
			unsigned long index;
			return _BitScanReverse64(&index, x)
				       ? (sizeof(unsigned long long) * CHAR_BIT - 1 - index)
		...
		constexpr inline int countl_zero(unsigned char x) noexcept {
			constexpr auto diff = std::numeric_limits<unsigned int>::digits - std::numeric_limits<unsigned
				                      char>::digits;
		...
		constexpr inline int countl_zero(fermat_uint128_t x) noexcept {
		...
		template<typename T, std::enable_if_t<std::is_unsigned_v<T> && sizeof(T) < 4, bool> = true>
		constexpr int countl_zero(const T num) noexcept {
			constexpr auto diff = 32 - fermat::numeric_limits<T>::digits;
		...
		template<typename T, std::enable_if_t<std::is_unsigned_v<T> && sizeof(T) == 8, bool> = true>
		constexpr int countl_zero(T x) noexcept {
			return countl_zero64(static_cast<uint64_t>(x));
		}
	...
	constexpr T bit_ceil(const T num) noexcept {
		...
		const auto shift = std::numeric_limits<T>::digits - fermat::countl_zero(static_cast<T>(num - 1));
	...
	constexpr T bit_floor(const T num) noexcept {
		...
		const auto shift = std::numeric_limits<T>::digits - fermat::countl_zero(num) - 1;
		return static_cast<T>(T(1) << shift);
	}

	template<typename T, typename = std::enable_if_t<std::is_unsigned_v<T> > >
	constexpr int bit_width(const T num) noexcept {
		return std::numeric_limits<T>::digits - fermat::countl_zero(num);
	}
	...
	namespace internal {
		...
		constexpr inline int popcount(fermat_uint128_t num) noexcept {
