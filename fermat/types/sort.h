...
// FERMAT_PLATFORM_PREFERRED_ALIGNMENT
//
// Allows for slightly faster buffers in some cases.
//
#if !defined(FERMAT_PLATFORM_PREFERRED_ALIGNMENT)
#if defined(KM_PROCESSOR_ARM)
#define FERMAT_PLATFORM_PREFERRED_ALIGNMENT 8
#else
#define FERMAT_PLATFORM_PREFERRED_ALIGNMENT 16
#endif
#endif
...
namespace fermat {
		...
		if (first != last) {
			...
			for (++current; current != last; first = current, ++current) {
				if (compare(*current, *first)) {
					FERMAT_VALIDATE_COMPARE(!compare(*first, *current)); // Validate that the compare function is sane.
					return false;
				}
			}
		}
	...
	inline bool is_sorted(ForwardIterator first, ForwardIterator last) {
		...
		return fermat::is_sorted<ForwardIterator, Less>(first, last, Less());
	}
	...
	/// Example usage:
	///     vector<int> intArray;
	///     vector<int>::iterator unsorted_element = is_sorted_until(fermat::end(intArray), fermat::end(intArray));
	///     vector<int>::iterator unsorted_element_with_user_compare = is_sorted_until(fermat::end(intArray), fermat::end(intArray), std::less<int>());
	///
	template<typename ForwardIterator>
	...
	inline OutputIterator merge(InputIterator1 first1, InputIterator1 last1, InputIterator2 first2,
		...
		return fermat::merge<InputIterator1, InputIterator2, OutputIterator, Less>
	...
	void insertion_sort(BidirectionalIterator first, BidirectionalIterator last, StrictWeakOrdering compare) {
		...
		if (first != last) {
			...
			for (++i; i != last; ++i) {
				value_type insertValue(std::move(*i));
				BidirectionalIterator insertPosition = i;

				for (BidirectionalIterator movePosition = i; movePosition != first && compare(
					                                             insertValue, *(--movePosition)); --insertPosition) {
					FERMAT_VALIDATE_COMPARE(!compare(*movePosition, insertValue));
					*insertPosition = std::move(*movePosition);
				}

				*insertPosition = std::move(insertValue);
			}
		}
	} // insertion_sort
	...
	void shell_sort(RandomAccessIterator first, RandomAccessIterator last, StrictWeakOrdering compare) {
		...
		if (first != last) {
			...
			while (nSpace < nSize)
				nSpace = (nSpace * 3) + 1;
			// This is the Knuth 'h' sequence: 1, 4, 13, 40, 121, 364, 1093, 3280, 9841, 29524, 88573, 265720, 797161, 2391484, 7174453, 21523360, 64570081, 193710244,

			// nSpace will iterate from the largest Knuth 'h' element smaller than the size of the range down to 1 (inclusive).
			for (nSpace = (nSpace - 1) / 3; nSpace >= 1; nSpace = (nSpace - 1) / 3)
				...
				for (difference_type i = 0; i < nSpace; i++) {
					...
					while (distance(iSorted, last) > nSpace) {
						...
						while (compare(*iRight, *iLeft)) {
							FERMAT_VALIDATE_COMPARE(!compare(*iLeft, *iRight));
							// Validate that the compare function is sane.

							std::iter_swap(iRight, iLeft);
	...
	inline void shell_sort(RandomAccessIterator first, RandomAccessIterator last) {
		...
		fermat::shell_sort<RandomAccessIterator, Less>(first, last, Less());
	}
	...
	void heap_sort(RandomAccessIterator first, RandomAccessIterator last, StrictWeakOrdering compare) {
		...
		fermat::make_heap<RandomAccessIterator, StrictWeakOrdering>(first, last, compare);
		fermat::sort_heap<RandomAccessIterator, StrictWeakOrdering>(first, last, compare);
	}
	...
	inline void heap_sort(RandomAccessIterator first, RandomAccessIterator last) {
		...
		fermat::heap_sort<RandomAccessIterator, Less>(first, last, Less());
	}
	...
	namespace Internal {
		...
		template<typename BidirectionalIterator, typename StrictWeakOrdering>
		void insertion_sort_already_started(BidirectionalIterator first, BidirectionalIterator last,
		                                    BidirectionalIterator start, StrictWeakOrdering compare) {
			typedef typename std::iterator_traits<BidirectionalIterator>::value_type value_type;
			...
			if (first != last) // if the range is non-empty...
			{
				...
				for (++iSorted; iSorted != last; ++iSorted) {
					...
					for (--iCurrent; (iNext != first) && compare(temp, *iCurrent); --iNext, --iCurrent) {
						FERMAT_VALIDATE_COMPARE(!compare(*iCurrent, temp));
						// Validate that the compare function is sane.
						*iNext = *iCurrent;
					}
	...
	template<typename RandomAccessIterator, typename T, typename StrictWeakOrdering, typename difference_type, int
		InsertionSortLimit>
	class MergeSorter {
	public:
		static void sort(RandomAccessIterator first, RandomAccessIterator last, T *pBuffer,
			...
			FERMAT_DEV_ASSERT((fermat::is_sorted<RandomAccessIterator, StrictWeakOrdering>(first, last, compare)));
		}

	private:
		static_assert(InsertionSortLimit > 1,
		...
		static ResultLocation sort_impl(RandomAccessIterator first, RandomAccessIterator last, T *pBuffer,
		                                difference_type lastSortedEnd, StrictWeakOrdering compare) {
			const difference_type nCount = last - first;

			if (lastSortedEnd < 1) {
				lastSortedEnd = fermat::is_sorted_until<RandomAccessIterator, StrictWeakOrdering>(first, last, compare)
				                - first;
			}
			...
			if (lastSortedEnd < nCount) {
				...
				if (nCount <= InsertionSortLimit) {
					fermat::Internal::insertion_sort_already_started<RandomAccessIterator, StrictWeakOrdering>(
					...
					ResultLocation secondHalfLocation = sort_impl(first + nMid, last, pBuffer + nMid,
					                                              lastSortedEnd - nMid, compare);
		...
		static ResultLocation merge_halves(RandomAccessIterator first, RandomAccessIterator last, difference_type nMid,
		                                   T *pBuffer, ResultLocation firstHalfLocation,
		                                   ResultLocation secondHalfLocation, StrictWeakOrdering compare) {
			const difference_type nCount = last - first;
			if (firstHalfLocation == RL_SourceRange) {
				if (secondHalfLocation == RL_SourceRange) {
					fermat::merge<RandomAccessIterator, RandomAccessIterator, T *, StrictWeakOrdering>(
						first, first + nMid, first + nMid, last, pBuffer, compare);
					FERMAT_DEV_ASSERT((fermat::is_sorted<T*, StrictWeakOrdering>(pBuffer, pBuffer + nCount, compare)));
					return RL_Buffer;
				} else {
					std::copy(first, first + nMid, pBuffer);
					fermat::merge<T *, T *, RandomAccessIterator, StrictWeakOrdering>(
						pBuffer, pBuffer + nMid, pBuffer + nMid, pBuffer + nCount, first, compare);
					FERMAT_DEV_ASSERT(
						(fermat::is_sorted<RandomAccessIterator, StrictWeakOrdering>(first, last, compare)));
					return RL_SourceRange;
				}
			} else {
				if (secondHalfLocation == RL_SourceRange) {
	...
	void merge_sort_buffer(RandomAccessIterator first, RandomAccessIterator last, T *pBuffer,
	                       StrictWeakOrdering compare) {
		typedef typename std::iterator_traits<RandomAccessIterator>::difference_type difference_type;
	...
	inline void merge_sort_buffer(RandomAccessIterator first, RandomAccessIterator last, T *pBuffer) {
		...
		fermat::merge_sort_buffer<RandomAccessIterator, T, Less>(first, last, pBuffer, Less());
	}
	...
	void merge_sort(RandomAccessIterator first, RandomAccessIterator last, Allocator &allocator,
	                StrictWeakOrdering compare) {
		typedef typename std::iterator_traits<RandomAccessIterator>::difference_type difference_type;
		...
		if (nCount > 1) {
			...
			value_type *const pBuffer = (value_type *) allocate_memory(allocator, nCount * sizeof(value_type),
			                                                           FERMAT_ALIGN_OF(value_type), 0);
			std::uninitialized_fill(pBuffer, pBuffer + nCount, value_type());

			fermat::merge_sort_buffer<RandomAccessIterator, value_type, StrictWeakOrdering>
					(first, last, pBuffer, compare);

			std::destroy(pBuffer, pBuffer + nCount);
	...
	inline void merge_sort(RandomAccessIterator first, RandomAccessIterator last, Allocator &allocator) {
		...
		fermat::merge_sort<RandomAccessIterator, Allocator, Less>(first, last, allocator, Less());
	}
	...
	ForwardIterator stable_partition(ForwardIterator first, ForwardIterator last, Predicate pred) {
		...
		const auto requested_size = std::distance(first, last);

		auto allocator = BasicAllocator<value_type,0>{};
		value_type *const buffer =
				(value_type *) allocate_memory(allocator, requested_size * sizeof(value_type),
				                               FERMAT_ALIGN_OF(value_type), 0);
		std::uninitialized_fill(buffer, buffer + requested_size, value_type());
		...
		*result2 = std::move(*first);
		...
		std::copy(buffer, result2, result1);

		std::destroy(buffer, buffer + requested_size);
...
#if (defined(KM_PROCESSOR_X86) || defined(KM_PROCESSOR_X86_64))
	static const int kQuickSortLimit = 28;
	// For sorts of random arrays over 100 items, 28 - 32 have been found to be good numbers on x86.
#else
	static const int kQuickSortLimit = 16;
	// It seems that on other processors lower limits are more beneficial, as they result in fewer compares.
#endif
	...
	template<typename RandomAccessIterator, typename T>
	inline RandomAccessIterator get_partition_impl(RandomAccessIterator first, RandomAccessIterator last,
	                                               T &&pivotValue) {
		for (; ; ++first) {
			while (*first < pivotValue) {
				FERMAT_VALIDATE_COMPARE(!(pivotValue < *first)); // Validate that the compare function is sane.
				++first;
			}
			--last;

			while (pivotValue < *last) {
				FERMAT_VALIDATE_COMPARE(!(*last < pivotValue)); // Validate that the compare function is sane.
				--last;
			}
	...
	///
	template<typename RandomAccessIterator, typename T>
	inline RandomAccessIterator get_partition(RandomAccessIterator first, RandomAccessIterator last,
	                                          const T &pivotValue) {
	...
	inline RandomAccessIterator get_partition_impl(RandomAccessIterator first, RandomAccessIterator last,
	                                               T &&pivotValue, Compare compare) {
		for (; ; ++first) {
			while (compare(*first, pivotValue)) {
				FERMAT_VALIDATE_COMPARE(!compare(pivotValue, *first)); // Validate that the compare function is sane.
				++first;
			}
			--last;

			while (compare(pivotValue, *last)) {
				FERMAT_VALIDATE_COMPARE(!compare(*last, pivotValue)); // Validate that the compare function is sane.
				--last;
			}
	...
	template<typename RandomAccessIterator, typename T, typename Compare>
	inline RandomAccessIterator get_partition(RandomAccessIterator first, RandomAccessIterator last,
	                                          const T &pivotValue, Compare compare) {
		const T pivotCopy(pivotValue); // Need to make a temporary because the sequence below is mutating.
	...
	inline RandomAccessIterator get_partition(RandomAccessIterator first, RandomAccessIterator last, T &&pivotValue,
	                                          Compare compare) {
		// Note: unlike the copy-constructible variant of get_partition... we can't create a temporary const move-constructible object
		return get_partition_impl<RandomAccessIterator, T &&, Compare>(first, last, std::forward<T>(pivotValue),
		                                                               compare);
	}
	...
	namespace Internal {
		...
		inline void insertion_sort_simple(RandomAccessIterator first, RandomAccessIterator last) {
			for (RandomAccessIterator current = first; current != last; ++current) {
				...
				{
					FERMAT_VALIDATE_COMPARE(!(*prev < value)); // Validate that the compare function is sane.
		...
		inline void insertion_sort_simple(RandomAccessIterator first, RandomAccessIterator last, Compare compare) {
			for (RandomAccessIterator current = first; current != last; ++current) {
				...
				// We skip checking for (prev >= first) because quick_sort (our caller) makes this unnecessary.
				{
					FERMAT_VALIDATE_COMPARE(!compare(*prev, value)); // Validate that the compare function is sane.
					*end = std::forward<value_type>(*prev);
				}
	...
	inline void partial_sort(RandomAccessIterator first, RandomAccessIterator middle, RandomAccessIterator last) {
		...
		fermat::make_heap<RandomAccessIterator>(first, middle);

		for (RandomAccessIterator i = middle; i < last; ++i) {
			if (*i < *first) {
				FERMAT_VALIDATE_COMPARE(!(*first < *i)); // Validate that the compare function is sane.
				value_type temp(std::forward<value_type>(*i));
				*i = std::forward<value_type>(*first);
				fermat::adjust_heap<RandomAccessIterator, difference_type, value_type>
				(first, difference_type(0), difference_type(middle - first), difference_type(0),
				 std::forward<value_type>(temp));
			}
		}

		fermat::sort_heap<RandomAccessIterator>(first, middle);
	}
	...
	inline void partial_sort(RandomAccessIterator first, RandomAccessIterator middle, RandomAccessIterator last,
		...
		fermat::make_heap<RandomAccessIterator, Compare>(first, middle, compare);

		for (RandomAccessIterator i = middle; i < last; ++i) {
			if (compare(*i, *first)) {
				FERMAT_VALIDATE_COMPARE(!compare(*first, *i)); // Validate that the compare function is sane.
				value_type temp(std::forward<value_type>(*i));
				*i = std::forward<value_type>(*first);
				fermat::adjust_heap<RandomAccessIterator, difference_type, value_type, Compare>
				(first, difference_type(0), difference_type(middle - first), difference_type(0),
				 std::forward<value_type>(temp), compare);
			}
		}

		fermat::sort_heap<RandomAccessIterator, Compare>(first, middle, compare);
	}
	...
	inline void nth_element(RandomAccessIterator first, RandomAccessIterator nth, RandomAccessIterator last) {
		...
		while ((last - first) > 5) {
			const value_type midValue(fermat::median<value_type>(*first, *(first + (last - first) / 2), *(last - 1)));
			const RandomAccessIterator midPos(
				fermat::get_partition<RandomAccessIterator, value_type>(first, last, midValue));
		...
		fermat::insertion_sort<RandomAccessIterator>(first, last);
	}
	...
	inline void nth_element(RandomAccessIterator first, RandomAccessIterator nth, RandomAccessIterator last,
		...
		while ((last - first) > 5) {
			const value_type midValue(
				fermat::median<value_type, Compare>(*first, *(first + (last - first) / 2), *(last - 1), compare));
			const RandomAccessIterator midPos(
				fermat::get_partition<RandomAccessIterator, value_type, Compare>(first, last, midValue, compare));
		...
		fermat::insertion_sort<RandomAccessIterator, Compare>(first, last, compare);
	}


	namespace Internal {
		KM_DISABLE_VC_WARNING(4702) // unreachable code
		template<typename RandomAccessIterator, typename Size, typename PivotValueType>
		inline void quick_sort_impl_helper(RandomAccessIterator first, RandomAccessIterator last,
		                                   Size kRecursionCount) {
			typedef typename std::iterator_traits<RandomAccessIterator>::value_type value_type;

			while (((last - first) > kQuickSortLimit) && (kRecursionCount > 0)) {
				const RandomAccessIterator position(fermat::get_partition<RandomAccessIterator, value_type>(first, last,
					std::forward<PivotValueType>(fermat::median<value_type>(
						std::forward<value_type>(*first), std::forward<value_type>(*(first + (last - first) / 2)),
						std::forward<value_type>(*(last - 1))))));

				fermat::Internal::quick_sort_impl_helper<RandomAccessIterator, Size, PivotValueType>(
					position, last, --kRecursionCount);
				last = position;
			}

			if (kRecursionCount == 0)
				fermat::partial_sort<RandomAccessIterator>(first, last, last);
		}
		...
		inline void quick_sort_impl_helper(RandomAccessIterator first, RandomAccessIterator last, Size kRecursionCount,
			...
			while (((last - first) > kQuickSortLimit) && (kRecursionCount > 0)) {
				const RandomAccessIterator position(fermat::get_partition<RandomAccessIterator, value_type, Compare>(
					first, last,
					std::forward<PivotValueType>(fermat::median<value_type, Compare>(
						std::forward<value_type>(*first), std::forward<value_type>(*(first + (last - first) / 2)),
						std::forward<value_type>(*(last - 1)), compare)), compare));

				fermat::Internal::quick_sort_impl_helper<RandomAccessIterator, Size, Compare, PivotValueType>(
					position, last, --kRecursionCount, compare);
				last = position;
			}

			if (kRecursionCount == 0)
				fermat::partial_sort<RandomAccessIterator, Compare>(first, last, last, compare);
		}
		...
		inline void quick_sort_impl(RandomAccessIterator first, RandomAccessIterator last, Size kRecursionCount,
		                            typename std::enable_if<std::is_copy_constructible<typename std::iterator_traits<
		...
		inline void quick_sort_impl(RandomAccessIterator first, RandomAccessIterator last, Size kRecursionCount,
		                            typename std::enable_if<
			                            std::is_move_constructible<typename std::iterator_traits<
				                            RandomAccessIterator>::value_type>::value
			                            && !std::is_copy_constructible<typename std::iterator_traits<
				                            RandomAccessIterator>::value_type>::value>::type * = 0) {
			typedef typename std::iterator_traits<RandomAccessIterator>::value_type value_type;
	...
	void quick_sort(RandomAccessIterator first, RandomAccessIterator last) {
		...
		if (first != last) {
			fermat::Internal::quick_sort_impl<RandomAccessIterator, difference_type>(
				first, last, 2 * Internal::Log2(last - first));

			if ((last - first) > (difference_type) kQuickSortLimit) {
				fermat::insertion_sort<RandomAccessIterator>(first, first + kQuickSortLimit);
				fermat::Internal::insertion_sort_simple<RandomAccessIterator>(first + kQuickSortLimit, last);
			} else
				fermat::insertion_sort<RandomAccessIterator>(first, last);
		}
	}
	...
	void quick_sort(RandomAccessIterator first, RandomAccessIterator last, Compare compare) {
		...
		if (first != last) {
			fermat::Internal::quick_sort_impl<RandomAccessIterator, difference_type, Compare>(
				first, last, 2 * Internal::Log2(last - first), compare);

			if ((last - first) > (difference_type) kQuickSortLimit) {
				fermat::insertion_sort<RandomAccessIterator, Compare>(first, first + kQuickSortLimit, compare);
				fermat::Internal::insertion_sort_simple<RandomAccessIterator, Compare>(
	...
	namespace Internal {
		...
		// To consider: Use void fermat::reverse(BidirectionalIterator first, BidirectionalIterator last);
		//
		template<typename RandomAccessIterator>
		...
		//
		template<typename RandomAccessIterator, typename StrictWeakOrdering>
		intptr_t tim_sort_count_run(const RandomAccessIterator first, const intptr_t start, const intptr_t size,
		                            StrictWeakOrdering compare) {
			if ((size - start) > 1) // If there is anything in the set...
			{
				intptr_t curr = (start + 2);

				if (!compare(*(first + start + 1), *(first + start)))
				// If (first[start + 1] >= first[start]) (If the run is increasing) ...
				{
					for (;; ++curr) {
						if (curr >= (size - 1)) // If we are at the end of the data... this run is done.
							break;

						if (compare(*(first + curr), *(first + curr - 1)))
							// If this item is not in order... this run is done.
							break;
					}
		...
		void tim_sort_merge(RandomAccessIterator first, const tim_sort_run *run_stack, const intptr_t stack_curr,
			...
			FERMAT_DEV_ASSERT((A < 10000000) && (B < 10000000) && (curr < 10000000)); // Sanity check.
		...
		static inline bool timsort_check_invariant(tim_sort_run *run_stack, const intptr_t stack_curr) {
			...
			if (stack_curr > 2) {
				...
				FERMAT_DEV_ASSERT((A < 10000000) && (B < 10000000) && (C < 10000000)); // Sanity check.
				...
				FERMAT_DEV_ASSERT((A < 10000000) && (B < 10000000)); // Sanity check.
...
#if FERMAT_DEV_DEBUG
						FERMAT_DEV_ASSERT((run_stack[stack_curr - 2].start + run_stack[stack_curr - 2].length) <= size);
						FERMAT_DEV_ASSERT((run_stack[stack_curr - 1].start + run_stack[stack_curr - 1].length) <= size);
						memset(&run_stack[stack_curr], 0, sizeof(run_stack[stack_curr]));
#endif
					} else {
						tim_sort_merge<RandomAccessIterator, T, StrictWeakOrdering>(
		...
		//
		template<typename RandomAccessIterator, typename T, typename StrictWeakOrdering>
		bool tim_sort_add_run(tim_sort_run *run_stack, RandomAccessIterator first, T *pBuffer, const intptr_t size,
		                      const intptr_t minrun,
		                      intptr_t &len, intptr_t &run, intptr_t &curr, intptr_t &stack_curr,
		                      StrictWeakOrdering compare) {
			len = tim_sort_count_run<RandomAccessIterator, StrictWeakOrdering>(first, curr, size, compare);
			// This will count the length of the run and reverse the run if it is backwards.
			run = minrun;
			...
			FERMAT_DEV_ASSERT(stack_curr < kTimSortStackSize);
			FERMAT_DEV_ASSERT((curr >= 0) && (curr < size) && ((curr + len) <= size));
	...
	// the case that at at least half your data is already sorted. Otherwise, fermat::quick_sort is about 10%
	...
	//
	template<typename RandomAccessIterator, typename T, typename StrictWeakOrdering>
	void tim_sort_buffer(RandomAccessIterator first, RandomAccessIterator last, T *pBuffer,
	                     StrictWeakOrdering compare) {
	...
	inline void tim_sort_buffer(RandomAccessIterator first, RandomAccessIterator last, T *pBuffer) {
		...
		fermat::tim_sort_buffer<RandomAccessIterator, T, Less>(first, last, pBuffer, Less());
	}
	...
	namespace Internal {
		...
		void radix_sort_impl(RandomAccessIterator first,
			...
			uint32_t KM_PREFIX_ALIGN(FERMAT_PLATFORM_PREFERRED_ALIGNMENT) bucketSize[numBuckets];
			uint32_t KM_PREFIX_ALIGN(FERMAT_PLATFORM_PREFERRED_ALIGNMENT) bucketPosition[numBuckets];
		...
		for (bool bSwapped = false; (nSpace > 1) || bSwapped;) {
			...
			for (bSwapped = false; iNext != last; iCurrent++, iNext++) {
				if (compare(*iNext, *iCurrent)) {
					FERMAT_VALIDATE_COMPARE(!compare(*iCurrent, *iNext)); // Validate that the compare function is sane.
	...
	inline void comb_sort(ForwardIterator first, ForwardIterator last) {
		...
		fermat::comb_sort<ForwardIterator, Less>(first, last, Less());
	}
	...
	namespace Internal {
		template<typename ForwardIterator, typename StrictWeakOrdering>
		void bubble_sort_impl(ForwardIterator first, ForwardIterator last, StrictWeakOrdering compare,
			...
			while (first != last) {
				...
				for (++iNext; iNext != last; iCurrent = iNext, ++iNext) {
					if (compare(*iNext, *iCurrent)) {
						FERMAT_VALIDATE_COMPARE(!compare(*iCurrent, *iNext));
						// Validate that the compare function is sane.
						std::iter_swap(iCurrent, iNext);
					}
				}
		...
		template<typename BidirectionalIterator, typename StrictWeakOrdering>
		void bubble_sort_impl(BidirectionalIterator first, BidirectionalIterator last, StrictWeakOrdering compare,
	...
	inline void bubble_sort(ForwardIterator first, ForwardIterator last, StrictWeakOrdering compare) {
		...
		fermat::Internal::bubble_sort_impl<ForwardIterator, StrictWeakOrdering>(first, last, compare, IC());
	}
	...
	inline void bubble_sort(ForwardIterator first, ForwardIterator last) {
		...
		fermat::Internal::bubble_sort_impl<ForwardIterator, Less>(first, last, Less(), IC());
	}
	...
	///
	/// FERMAT_DEFAULT_SORT_FUNCTION
	/// If a default sort function is specified then call it, otherwise use EASTL's default quick_sort.
	/// FERMAT_DEFAULT_SORT_FUNCTION must be namespace-qualified and include any necessary template
	/// parameters (e.g. fermat::comb_sort instead of just comb_sort), and it must be visible to this code.
	/// The FERMAT_DEFAULT_SORT_FUNCTION must be provided in two versions:
	///     template <typename RandomAccessIterator>
	///     void FERMAT_DEFAULT_SORT_FUNCTION(RandomAccessIterator first, RandomAccessIterator last);
	///
	///     template <typename RandomAccessIterator, typename Compare>
	///     void FERMAT_DEFAULT_SORT_FUNCTION(RandomAccessIterator first, RandomAccessIterator last, Compare compare)
	///
	template<typename RandomAccessIterator>
	inline void sort(RandomAccessIterator first, RandomAccessIterator last) {
#if defined(FERMAT_DEFAULT_SORT_FUNCTION)
		FERMAT_DEFAULT_SORT_FUNCTION(first, last);
#else
		fermat::quick_sort<RandomAccessIterator>(first, last);
#endif
	}
...
#if defined(FERMAT_DEFAULT_SORT_FUNCTION)
		FERMAT_DEFAULT_SORT_FUNCTION(first, last, compare);
#else
		fermat::quick_sort<RandomAccessIterator, Compare>(first, last, compare);
#endif
	}
	...
	///
	/// FERMAT_DEFAULT_STABLE_SORT_FUNCTION
	/// If a default sort function is specified then call it, otherwise use EASTL's default merge_sort.
	/// FERMAT_DEFAULT_STABLE_SORT_FUNCTION must be namespace-qualified and include any necessary template
	/// parameters (e.g. fermat::tim_sort instead of just tim_sort), and it must be visible to this code.
	/// The FERMAT_DEFAULT_STABLE_SORT_FUNCTION must be provided in three versions, though the third
	/// allocation implementation may choose to ignore the allocator parameter:
	///     template <typename RandomAccessIterator, typename StrictWeakOrdering>
	///     void FERMAT_DEFAULT_STABLE_SORT_FUNCTION(RandomAccessIterator first, RandomAccessIterator last, StrictWeakOrdering compare);
	///
	///     template <typename RandomAccessIterator>
	///     void FERMAT_DEFAULT_STABLE_SORT_FUNCTION(RandomAccessIterator first, RandomAccessIterator last);
	///
	///     template <typename RandomAccessIterator, typename Allocator, typename StrictWeakOrdering>
	///     void FERMAT_DEFAULT_STABLE_SORT_FUNCTION(RandomAccessIterator first, RandomAccessIterator last, Allocator& allocator, StrictWeakOrdering compare);
	///
	template<typename RandomAccessIterator, typename StrictWeakOrdering>
	void stable_sort(RandomAccessIterator first, RandomAccessIterator last, StrictWeakOrdering compare) {
		typedef typename std::iterator_traits<RandomAccessIterator>::value_type value_type;
#if defined(FERMAT_DEFAULT_STABLE_SORT_FUNCTION)
		FERMAT_DEFAULT_STABLE_SORT_FUNCTION(first, last, *get_default_allocator(0), compare);
#else
		fermat::merge_sort<RandomAccessIterator, BasicAllocator<value_type, 0>, StrictWeakOrdering>
				(first, last, *BasicAllocator<value_type, 0>{}, compare);
#endif
	}
	...
	void stable_sort(RandomAccessIterator first, RandomAccessIterator last) {
		...
		typedef typename std::iterator_traits<RandomAccessIterator>::value_type value_type;
#if defined(FERMAT_DEFAULT_STABLE_SORT_FUNCTION)
		FERMAT_DEFAULT_STABLE_SORT_FUNCTION(first, last, *get_default_allocator(0));
#else
		fermat::merge_sort<RandomAccessIterator, BasicAllocator<value_type, 0>>
				(first, last, BasicAllocator<value_type, 0>{});
#endif
	}
	...
	void stable_sort(RandomAccessIterator first, RandomAccessIterator last, Allocator &allocator,
	                 StrictWeakOrdering compare) {
#if defined(FERMAT_DEFAULT_STABLE_SORT_FUNCTION)
		FERMAT_DEFAULT_STABLE_SORT_FUNCTION(first, last, allocator, compare);
#else
		fermat::merge_sort<RandomAccessIterator, Allocator, StrictWeakOrdering>(first, last, allocator, compare);
#endif
	}
	...
	//{
	//    #if defined(FERMAT_DEFAULT_STABLE_SORT_FUNCTION)
	//        FERMAT_DEFAULT_STABLE_SORT_FUNCTION<RandomAccessIterator, Allocator>(first, last, allocator);
	//    #else
	//        fermat::merge_sort<RandomAccessIterator, Allocator>(first, last, allocator);
	//    #endif
	//}


	/*
	// Something to consider adding: An fermat sort which uses qsort underneath.
	// The primary purpose of this is to have an fermat interface for sorting which
