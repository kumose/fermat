...
#ifndef FERMAT_META_H
#define FERMAT_META_H
	...
	namespace meta {
		...
		namespace Internal {
			...
			struct get_type_index<I, T, Head, Types...> {
				static constexpr int value = std::is_same_v<T, Head> ? I : get_type_index<I + 1, T, Types...>::value;
			};
		...
		FERMAT_CPP17_INLINE_VARIABLE constexpr int get_type_index_v = get_type_index<T, Ts...>::value;
		...
		struct type_count<T, H, Types...> {
			static constexpr int value = (std::is_same_v<T, H> ? 1 : 0) + type_count<T, Types...>::value;
		};
		...
		template<typename T, typename... Ts>
		FERMAT_CPP17_INLINE_VARIABLE constexpr int type_count_v = type_count<T, Ts...>::value;
		...
		struct duplicate_type_check {
			static constexpr bool value = (type_count<T, Types...>::value == 1);
		};

		template<typename... Ts>
		FERMAT_CPP17_INLINE_VARIABLE constexpr bool duplicate_type_check_v = duplicate_type_check<Ts...>::value;
		...
		struct unique_type_list : public unique_type_list<Ts...> {
			...
			static std::enable_if_t<!std::disjunction_v<std::is_same<T, Args>...>, type_list<T, Args...> >
			types(type_list<Args...>);

			template<typename... Args>
			static std::enable_if_t<std::disjunction_v<std::is_same<T, Args>...>, type_list<Args...> >
			types(type_list<Args...>);

			typedef decltype(types(std::declval<typename unique_type_list<Ts...>::type>())) type;
		};
		...
		KM_DISABLE_VC_WARNING(4242 4244) // conversion from 'T' to 'U', possible loss of data.
		template<typename T, typename OverloadSet, typename ResultT = decltype(std::declval<OverloadSet>()(
			std::declval<T>()))>
		...
		using overload_resolution_t = typename overload_resolution<std::decay_t<T>, OverloadSet>::type;
		...
		struct double_pack_expansion<std::index_sequence<Is...>, I> {
