...
#ifndef FERMAT_NUMERIC_H
#define FERMAT_NUMERIC_H
...
namespace fermat {
	...
	///
    template<typename InputIterator1, typename InputIterator2, typename T, typename BinaryOperation1, typename
        BinaryOperation2>
    T inner_product(InputIterator1 first1, InputIterator1 last1, InputIterator2 first2, T init,
...
#if defined(KM_COMPILER_CPP20_ENABLED)
namespace fermat {
    template<typename T, std::enable_if_t<is_arithmetic_v<T> && !std::is_same_v<std::remove_cv_t<T>, bool>, int> = 0>
    constexpr T midpoint(T lhs, T rhs) noexcept {
        // If T is an integral type
        if constexpr (fermat::is_integral_v<T>) {
        ...
        // otherwise if T is a floating point
            ...
            if (fermat::is_constant_evaluated()) {
            ...
            constexpr T hi = fermat::numeric_limits<T>::max() / 2;
            ...
            constexpr T lo = fermat::numeric_limits<T>::min() * 2;
            if (lhs_abs < lo) {
                return lhs + rhs / 2;
            } else if (rhs_abs < lo) {
                return lhs / 2 + rhs;
...
namespace fermat {
    namespace internal {
        template<class T>
        constexpr T shared_lerp(const T a, const T b, const T t) noexcept {
            if ((a <= 0 && b >= 0) || (a >= 0 && b <= 0)) {
                return t * b + (1 - t) * a;
            }
	...
	///
    constexpr float lerp(float a, float b, float t) noexcept { return internal::shared_lerp(a, b, t); }
    constexpr double lerp(double a, double b, double t) noexcept { return internal::shared_lerp(a, b, t); }

    constexpr long double lerp(long double a, long double b, long double t) noexcept {
        return internal::shared_lerp(a, b, t);
    }
} // namespace fermat
