...
KM_DISABLE_VC_WARNING(4267 4345 4480 4530 4571);
...
KM_DISABLE_VC_WARNING(4703 4701)
#endif

namespace fermat {
	/// FERMAT_DEQUE_DEFAULT_NAME
	///
	/// Defines a default container name in the absence of a user-provided name.
	///
#ifndef FERMAT_DEQUE_DEFAULT_NAME
#define FERMAT_DEQUE_DEFAULT_NAME FERMAT_DEFAULT_NAME_PREFIX " deque" // Unless the user overrides something, this is "EASTL deque".

#endif


	/// FERMAT_DEQUE_DEFAULT_ALLOCATOR
	///
#ifndef FERMAT_DEQUE_DEFAULT_ALLOCATOR
#define FERMAT_DEQUE_DEFAULT_ALLOCATOR allocator_type(FERMAT_DEQUE_DEFAULT_NAME)
#endif
...
#define DEQUE_DEFAULT_SUBARRAY_SIZE(T) 16
	...
	protected:
		...
		template<typename, size_t, typename, unsigned>
	...
	///
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	struct DequeBase {
		typedef T value_type;
		...
		enum {
			...
			//kNodeSize        = kDequeSubarraySize * sizeof(T)   /// Disabled because it prevents the ability to do this: struct X{ fermat::deque<X, EASTLAllocatorType, 16> mDequeOfSelf; };
		};
	...
	///
	template<typename T, size_t Alignment, typename Allocator = BasicAllocator<T, Alignment>, unsigned kDequeSubarraySize =
			DEQUE_DEFAULT_SUBARRAY_SIZE(T)>
	class deque : public DequeBase<T, Alignment, Allocator, kDequeSubarraySize> {
	public:
		typedef DequeBase<T, Alignment, Allocator, kDequeSubarraySize> base_type;
		typedef deque<T, Alignment, Allocator, kDequeSubarraySize> this_type;
		typedef T value_type;
		typedef T *pointer;
	...
	public:
		...
		explicit deque(size_type n, const allocator_type &allocator = FERMAT_DEQUE_DEFAULT_ALLOCATOR);

		deque(size_type n, const value_type &value, const allocator_type &allocator = FERMAT_DEQUE_DEFAULT_ALLOCATOR);
		...
		deque(std::initializer_list<value_type> ilist,
		      const allocator_type &allocator = FERMAT_DEQUE_DEFAULT_ALLOCATOR);
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	DequeBase<T, Alignment, Allocator, kDequeSubarraySize>::DequeBase(const allocator_type &allocator)
		: mpPtrArray(nullptr),
		  mnPtrArraySize(0),
		  mItBegin(),
		  mItEnd(),
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	DequeBase<T, Alignment, Allocator, kDequeSubarraySize>::DequeBase(size_type n)
		: mpPtrArray(nullptr),
		  mnPtrArraySize(0),
		  mItBegin(),
		  mItEnd(),
		  mAllocator(FERMAT_DEQUE_DEFAULT_NAME) {
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	DequeBase<T, Alignment, Allocator, kDequeSubarraySize>::DequeBase(size_type n, const allocator_type &allocator)
		: mpPtrArray(nullptr),
		  mnPtrArraySize(0),
		  mItBegin(),
		  mItEnd(),
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	DequeBase<T, Alignment, Allocator, kDequeSubarraySize>::~DequeBase() {
		if (mpPtrArray) {
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	const typename DequeBase<T, Alignment, Allocator, kDequeSubarraySize>::allocator_type &
	DequeBase<T, Alignment, Allocator, kDequeSubarraySize>::get_allocator() const noexcept {
		return mAllocator;
	}


	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	typename DequeBase<T, Alignment, Allocator, kDequeSubarraySize>::allocator_type &
	DequeBase<T, Alignment, Allocator, kDequeSubarraySize>::get_allocator() noexcept {
		return mAllocator;
	}


	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	void DequeBase<T, Alignment, Allocator, kDequeSubarraySize>::set_allocator(const allocator_type &allocator) {
		// The only time you can set an allocator is with an empty unused container, such as right after construction.
		if (FERMAT_LIKELY(mAllocator != allocator)) {
			...
			if (FERMAT_LIKELY(mpPtrArray && (mItBegin.mpCurrent == mItEnd.mpCurrent))) // is the container empty?
			{
				DoFreeSubarrays(mItBegin.mpCurrentArrayPtr, mItEnd.mpCurrentArrayPtr + 1);
				DoFreePtrArray(mpPtrArray, mnPtrArraySize);
				...
				FERMAT_THROW_MSG_OR_ASSERT(std::logic_error,
				                           "deque::set_allocator -- attempt to change allocator after inserting elements.");
			}
		}
	}
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	T *DequeBase<T, Alignment, Allocator, kDequeSubarraySize>::DoAllocateSubarray() {
		T *p = (T *) allocate_memory(mAllocator, kDequeSubarraySize * sizeof(T), FERMAT_ALIGN_OF(T), 0);
		FERMAT_ASSERT_MSG(p != nullptr, "the behaviour of fermat::allocators that return nullptr is not defined.");
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	void DequeBase<T, Alignment, Allocator, kDequeSubarraySize>::DoFreeSubarray(T *p) {
		if (p)
			EASTLFree(mAllocator, p, kDequeSubarraySize * sizeof(T));
	}

	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	void DequeBase<T, Alignment, Allocator, kDequeSubarraySize>::DoFreeSubarrays(T **pBegin, T **pEnd) {
		while (pBegin < pEnd)
			DoFreeSubarray(*pBegin++);
	}

	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	T **DequeBase<T, Alignment, Allocator, kDequeSubarraySize>::DoAllocatePtrArray(size_type n) {
#if FERMAT_ASSERT_ENABLED
		if (FERMAT_UNLIKELY(n >= 0x80000000))
			FERMAT_FAIL_MSG("deque::DoAllocatePtrArray -- improbably large request.");
#endif

		T **pp = (T **) allocate_memory(mAllocator, n * sizeof(T *), FERMAT_ALIGN_OF(T), 0);
		FERMAT_ASSERT_MSG(pp != nullptr, "the behaviour of fermat::allocators that return nullptr is not defined.");
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	void DequeBase<T, Alignment, Allocator, kDequeSubarraySize>::DoFreePtrArray(T **pp, size_t n) {
		if (pp)
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	typename DequeBase<T, Alignment, Allocator, kDequeSubarraySize>::iterator
	DequeBase<T, Alignment, Allocator, kDequeSubarraySize>::DoReallocSubarray(size_type nAdditionalCapacity, Side allocationSide) {
		...
		if (allocationSide == kSideFront) {
			...
			if (FERMAT_UNLIKELY(nCurrentAdditionalCapacity < nAdditionalCapacity))
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	void DequeBase<T, Alignment, Allocator, kDequeSubarraySize>::DoReallocPtrArray(size_type nAdditionalCapacity,
	                                                                    Side allocationSide) {
		...
		// If we can take advantage of unused pointers at the back without doing any reallocation...
		{
			...
			const size_type nNewPtrArraySize = mnPtrArraySize + fermat::max_alt(mnPtrArraySize, nAdditionalCapacity) +
			                                   2; // Allocate extra capacity.
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	void DequeBase<T, Alignment, Allocator, kDequeSubarraySize>::DoInit(size_type n) {
		...
		mnPtrArraySize = fermat::max_alt(kMinPtrArraySize_, (nNewPtrArraySize + 2));
		...
		//    mnPtrArraySize = 0;
		//
		//    mItBegin.mpCurrentArrayPtr = nullptr;
		//    mItBegin.mpBegin           = nullptr;
		//    mItBegin.mpEnd             = nullptr; // We intentionally create a situation whereby the subarray that has no capacity.
		//    mItBegin.mpCurrent         = nullptr;
		//
		//    mItEnd = mItBegin;
		//}
	}
	...
	DequeIterator<T, Pointer, Reference, kDequeSubarraySize>::DequeIterator()
		: mpCurrent(nullptr), mpBegin(nullptr), mpEnd(nullptr), mpCurrentArrayPtr(nullptr) {
		// Empty
	}
	...
	DequeIterator<T, Pointer, Reference, kDequeSubarraySize>::operator++() {
		if (FERMAT_UNLIKELY(++mpCurrent == mpEnd)) {
	...
	typename DequeIterator<T, Pointer, Reference, kDequeSubarraySize>::this_type &
	DequeIterator<T, Pointer, Reference, kDequeSubarraySize>::operator--() {
		if (FERMAT_UNLIKELY(mpCurrent == mpBegin)) {
			mpBegin = *--mpCurrentArrayPtr;
			mpEnd = mpBegin + kDequeSubarraySize;
	...
	DequeIterator<T, Pointer, Reference, kDequeSubarraySize>::operator+=(difference_type n) {
		...
		// If the new position is within the current subarray (i.e. >= 0 && < kSubArraySize)...
			...
			FERMAT_CT_ASSERT((kDequeSubarraySize & (kDequeSubarraySize - 1)) == 0); // Verify that it is a power of 2.
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	inline deque<T, Alignment, Allocator, kDequeSubarraySize>::deque()
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	inline deque<T, Alignment, Allocator, kDequeSubarraySize>::deque(const allocator_type &allocator)
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	inline deque<T, Alignment, Allocator, kDequeSubarraySize>::deque(size_type n, const allocator_type &allocator)
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	inline deque<T, Alignment, Allocator, kDequeSubarraySize>::deque(size_type n, const value_type &value,
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	inline deque<T, Alignment, Allocator, kDequeSubarraySize>::deque(const this_type &x)
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	inline deque<T, Alignment, Allocator, kDequeSubarraySize>::deque(this_type &&x)
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	inline deque<T, Alignment, Allocator, kDequeSubarraySize>::deque(this_type &&x, const allocator_type &allocator)
		: base_type((size_type) 0, allocator) {
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	inline deque<T, Alignment, Allocator, kDequeSubarraySize>::deque(std::initializer_list<value_type> ilist,
	                                                      const allocator_type &allocator)
		: base_type(allocator) {
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	template<typename InputIterator>
	inline deque<T, Alignment, Allocator, kDequeSubarraySize>::deque(InputIterator first, InputIterator last)
		: base_type(FERMAT_DEQUE_DEFAULT_ALLOCATOR)
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	typename deque<T, Alignment, Allocator, kDequeSubarraySize>::this_type &
	deque<T, Alignment, Allocator, kDequeSubarraySize>::operator=(const this_type &x) {
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	inline typename deque<T, Alignment, Allocator, kDequeSubarraySize>::this_type &
	deque<T, Alignment, Allocator, kDequeSubarraySize>::operator=(this_type &&x) {
		if (this != &x) {
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	inline typename deque<T, Alignment, Allocator, kDequeSubarraySize>::this_type &
	deque<T, Alignment, Allocator, kDequeSubarraySize>::operator=(std::initializer_list<value_type> ilist) {
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	inline void deque<T, Alignment, Allocator, kDequeSubarraySize>::assign(size_type n, const value_type &value) {
		DoAssignValues(n, value);
	}


	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	inline void deque<T, Alignment, Allocator, kDequeSubarraySize>::assign(std::initializer_list<value_type> ilist) {
		DoAssign(ilist.begin(), ilist.end(), std::false_type());
	}
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	template<typename InputIterator>
	inline void deque<T, Alignment, Allocator, kDequeSubarraySize>::assign(InputIterator first, InputIterator last) {
		DoAssign(first, last, std::is_integral<InputIterator>());
	}


	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	inline typename deque<T, Alignment, Allocator, kDequeSubarraySize>::iterator
	deque<T, Alignment, Allocator, kDequeSubarraySize>::begin() noexcept {
		return mItBegin;
	}


	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	inline typename deque<T, Alignment, Allocator, kDequeSubarraySize>::const_iterator
	deque<T, Alignment, Allocator, kDequeSubarraySize>::begin() const noexcept {
		return mItBegin;
	}
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	inline typename deque<T, Alignment, Allocator, kDequeSubarraySize>::const_iterator
	deque<T, Alignment, Allocator, kDequeSubarraySize>::cbegin() const noexcept {
		return mItBegin;
	}
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	inline typename deque<T, Alignment, Allocator, kDequeSubarraySize>::iterator
	deque<T, Alignment, Allocator, kDequeSubarraySize>::end() noexcept {
		return mItEnd;
	}


	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	typename deque<T, Alignment, Allocator, kDequeSubarraySize>::const_iterator
	deque<T, Alignment, Allocator, kDequeSubarraySize>::end() const noexcept {
		return mItEnd;
	}
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	inline typename deque<T, Alignment, Allocator, kDequeSubarraySize>::const_iterator
	deque<T, Alignment, Allocator, kDequeSubarraySize>::cend() const noexcept {
		return mItEnd;
	}


	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	inline typename deque<T, Alignment, Allocator, kDequeSubarraySize>::reverse_iterator
	deque<T, Alignment, Allocator, kDequeSubarraySize>::rbegin() noexcept {
		return reverse_iterator(mItEnd);
	}


	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	inline typename deque<T, Alignment, Allocator, kDequeSubarraySize>::const_reverse_iterator
	deque<T, Alignment, Allocator, kDequeSubarraySize>::rbegin() const noexcept {
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	inline typename deque<T, Alignment, Allocator, kDequeSubarraySize>::const_reverse_iterator
	deque<T, Alignment, Allocator, kDequeSubarraySize>::crbegin() const noexcept {
		return const_reverse_iterator(mItEnd);
	}
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	inline typename deque<T, Alignment, Allocator, kDequeSubarraySize>::reverse_iterator
	deque<T, Alignment, Allocator, kDequeSubarraySize>::rend() noexcept {
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	inline typename deque<T, Alignment, Allocator, kDequeSubarraySize>::const_reverse_iterator
	deque<T, Alignment, Allocator, kDequeSubarraySize>::rend() const noexcept {
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	inline typename deque<T, Alignment, Allocator, kDequeSubarraySize>::const_reverse_iterator
	deque<T, Alignment, Allocator, kDequeSubarraySize>::crend() const noexcept {
		return const_reverse_iterator(mItBegin);
	}


	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	inline bool deque<T, Alignment, Allocator, kDequeSubarraySize>::empty() const noexcept {
		return mItBegin.mpCurrent == mItEnd.mpCurrent;
	}


	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	typename deque<T, Alignment, Allocator, kDequeSubarraySize>::size_type
	inline deque<T, Alignment, Allocator, kDequeSubarraySize>::size() const noexcept {
		return (size_type) (mItEnd - mItBegin);
	}


	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	inline void deque<T, Alignment, Allocator, kDequeSubarraySize>::resize(size_type n, const value_type &value) {
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	inline void deque<T, Alignment, Allocator, kDequeSubarraySize>::resize(size_type n) {
		resize(n, value_type());
	}


	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	inline void deque<T, Alignment, Allocator, kDequeSubarraySize>::shrink_to_fit() {
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	inline void deque<T, Alignment, Allocator, kDequeSubarraySize>::set_capacity(size_type n) {
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	typename deque<T, Alignment, Allocator, kDequeSubarraySize>::reference
	deque<T, Alignment, Allocator, kDequeSubarraySize>::operator[](size_type n) {
#if FERMAT_ASSERT_ENABLED && FERMAT_EMPTY_REFERENCE_ASSERT_ENABLED
		if (FERMAT_UNLIKELY(n >= (size_type)(mItEnd - mItBegin)))
			FERMAT_FAIL_MSG("deque::operator[] -- out of range");
#elif FERMAT_ASSERT_ENABLED
		// We allow taking a reference to deque[0]
		if (FERMAT_UNLIKELY((n != 0) && n >= (size_type)(mItEnd - mItBegin)))
			FERMAT_FAIL_MSG("deque::operator[] -- out of range");
#endif
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	typename deque<T, Alignment, Allocator, kDequeSubarraySize>::const_reference
	deque<T, Alignment, Allocator, kDequeSubarraySize>::operator[](size_type n) const {
#if FERMAT_ASSERT_ENABLED && FERMAT_EMPTY_REFERENCE_ASSERT_ENABLED
		if (FERMAT_UNLIKELY(n >= (size_type)(mItEnd - mItBegin)))
			FERMAT_FAIL_MSG("deque::operator[] -- out of range");
#elif FERMAT_ASSERT_ENABLED
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	typename deque<T, Alignment, Allocator, kDequeSubarraySize>::reference
	deque<T, Alignment, Allocator, kDequeSubarraySize>::at(size_type n) {
#if FERMAT_EXCEPTIONS_ENABLED
		if (n >= (size_type) (mItEnd - mItBegin))
			throw std::out_of_range("deque::at -- out of range");
#elif FERMAT_ASSERT_ENABLED
		if (n >= (size_type) (mItEnd - mItBegin))
			FERMAT_FAIL_MSG("deque::at -- out of range");
#endif
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	typename deque<T, Alignment, Allocator, kDequeSubarraySize>::const_reference
	deque<T, Alignment, Allocator, kDequeSubarraySize>::at(size_type n) const {
#if FERMAT_EXCEPTIONS_ENABLED
		if (n >= (size_type) (mItEnd - mItBegin))
			throw std::out_of_range("deque::at -- out of range");
#elif FERMAT_ASSERT_ENABLED
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	typename deque<T, Alignment, Allocator, kDequeSubarraySize>::reference
	deque<T, Alignment, Allocator, kDequeSubarraySize>::front() {
#if FERMAT_ASSERT_ENABLED && FERMAT_EMPTY_REFERENCE_ASSERT_ENABLED
		if (FERMAT_UNLIKELY((size_type)(mItEnd == mItBegin)))
			FERMAT_FAIL_MSG("deque::front -- empty deque");
#else
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	typename deque<T, Alignment, Allocator, kDequeSubarraySize>::const_reference
	deque<T, Alignment, Allocator, kDequeSubarraySize>::front() const {
#if FERMAT_ASSERT_ENABLED && FERMAT_EMPTY_REFERENCE_ASSERT_ENABLED
		if (FERMAT_UNLIKELY((size_type)(mItEnd == mItBegin)))
			FERMAT_FAIL_MSG("deque::front -- empty deque");
#else
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	typename deque<T, Alignment, Allocator, kDequeSubarraySize>::reference
	deque<T, Alignment, Allocator, kDequeSubarraySize>::back() {
#if FERMAT_ASSERT_ENABLED
		...
		if (FERMAT_UNLIKELY((size_type)(mItEnd == mItBegin)))
			FERMAT_FAIL_MSG("deque::back -- empty deque");
#endif
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	typename deque<T, Alignment, Allocator, kDequeSubarraySize>::const_reference
	deque<T, Alignment, Allocator, kDequeSubarraySize>::back() const {
#if FERMAT_ASSERT_ENABLED
		// Decrementing an iterator with an empty container will result in undefined behaviour.
		// specifically: the iterator decrement will apply pointer arithmetic to a nullptr (depending on the situation either mpCurrentArrayPtr or mpBegin).
		if (FERMAT_UNLIKELY((size_type)(mItEnd == mItBegin)))
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	void deque<T, Alignment, Allocator, kDequeSubarraySize>::push_front(const value_type &value) {
		emplace_front(value);
	}


	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	void deque<T, Alignment, Allocator, kDequeSubarraySize>::push_front(value_type &&value) {
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	typename deque<T, Alignment, Allocator, kDequeSubarraySize>::reference
	deque<T, Alignment, Allocator, kDequeSubarraySize>::push_front() {
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	void deque<T, Alignment, Allocator, kDequeSubarraySize>::push_back(const value_type &value) {
		emplace_back(value);
	}


	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	void deque<T, Alignment, Allocator, kDequeSubarraySize>::push_back(value_type &&value) {
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	typename deque<T, Alignment, Allocator, kDequeSubarraySize>::reference
	deque<T, Alignment, Allocator, kDequeSubarraySize>::push_back() {
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	void deque<T, Alignment, Allocator, kDequeSubarraySize>::pop_front() {
#if FERMAT_ASSERT_ENABLED
		if (FERMAT_UNLIKELY((size_type)(mItEnd == mItBegin)))
			FERMAT_FAIL_MSG("deque::pop_front -- empty deque");
#endif
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	void deque<T, Alignment, Allocator, kDequeSubarraySize>::pop_back() {
#if FERMAT_ASSERT_ENABLED
		if (FERMAT_UNLIKELY((size_type)(mItEnd == mItBegin)))
			FERMAT_FAIL_MSG("deque::pop_back -- empty deque");
#endif
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	template<class... Args>
	typename deque<T, Alignment, Allocator, kDequeSubarraySize>::iterator
	deque<T, Alignment, Allocator, kDequeSubarraySize>::emplace(const_iterator position, Args &&... args) {
		if (FERMAT_UNLIKELY(position.mpCurrent == mItEnd.mpCurrent)) // If we are doing the same thing as push_back...
		{
		...
		} else if (FERMAT_UNLIKELY(position.mpCurrent == mItBegin.mpCurrent))
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	template<class... Args>
	typename deque<T, Alignment, Allocator, kDequeSubarraySize>::reference deque<T, Alignment, Allocator, kDequeSubarraySize>::emplace_front(
		Args &&... args) {
		...
		// If we have room in the first subarray... we hope that usually this 'new' pathway gets executed, as it is slightly faster.
			detail::allocator_construct(mAllocator, --mItBegin.mpCurrent, std::forward<Args>(args)...);
		else {
			...
			detail::allocator_construct(mAllocator, mItBegin.mpCurrent, std::move(valueSaved));
#if FERMAT_EXCEPTIONS_ENABLED
				}
				catch(...) {
				++mItBegin;
				// The exception could only occur in the new operation above, after we have incremented mItBegin. So we need to undo it.
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	template<class... Args>
	typename deque<T, Alignment, Allocator, kDequeSubarraySize>::reference deque<T, Alignment, Allocator, kDequeSubarraySize>::emplace_back(
		Args &&... args) {
		...
		// If we have room in the last subarray... we hope that usually this 'new' pathway gets executed, as it is slightly faster.
		{
			...
			detail::allocator_construct(mAllocator, mItEnd.mpCurrent++, std::forward<Args>(args)...);
			return back;
		} else {
			// To consider: Detect if value isn't coming from within this container and handle that efficiently.
			value_type valueSaved(std::forward<Args>(args)...);
			...
			try {
#endif
			detail::allocator_construct(mAllocator, mItEnd.mpCurrent, std::move(valueSaved));
			mItEnd.SetSubarray(mItEnd.mpCurrentArrayPtr + 1);
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	typename deque<T, Alignment, Allocator, kDequeSubarraySize>::iterator
	deque<T, Alignment, Allocator, kDequeSubarraySize>::insert(const_iterator position, const value_type &value) {
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	typename deque<T, Alignment, Allocator, kDequeSubarraySize>::iterator
	deque<T, Alignment, Allocator, kDequeSubarraySize>::insert(const_iterator position, value_type &&value) {
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	typename deque<T, Alignment, Allocator, kDequeSubarraySize>::iterator
	deque<T, Alignment, Allocator, kDequeSubarraySize>::insert(const_iterator position, size_type n, const value_type &value) {
	...
	template<typename InputIterator>
	typename deque<T, Alignment, Allocator, kDequeSubarraySize>::iterator
	deque<T, Alignment, Allocator, kDequeSubarraySize>::insert(const_iterator position, InputIterator first, InputIterator last) {
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	typename deque<T, Alignment, Allocator, kDequeSubarraySize>::iterator
	deque<T, Alignment, Allocator, kDequeSubarraySize>::insert(const_iterator position, std::initializer_list<value_type> ilist) {
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	typename deque<T, Alignment, Allocator, kDequeSubarraySize>::iterator
	deque<T, Alignment, Allocator, kDequeSubarraySize>::erase(const_iterator position) {
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	typename deque<T, Alignment, Allocator, kDequeSubarraySize>::iterator
	deque<T, Alignment, Allocator, kDequeSubarraySize>::erase(const_iterator first, const_iterator last) {
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	typename deque<T, Alignment, Allocator, kDequeSubarraySize>::reverse_iterator
	deque<T, Alignment, Allocator, kDequeSubarraySize>::erase(reverse_iterator position) {
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	typename deque<T, Alignment, Allocator, kDequeSubarraySize>::reverse_iterator
	deque<T, Alignment, Allocator, kDequeSubarraySize>::erase(reverse_iterator first, reverse_iterator last) {
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	void deque<T, Alignment, Allocator, kDequeSubarraySize>::clear() {
	...
	//template <typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	//void deque<T, Alignment, Allocator, kDequeSubarraySize>::reset_lose_memory()
	//{
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	void deque<T, Alignment, Allocator, kDequeSubarraySize>::swap(deque &x) {
#if defined(FERMAT_DEQUE_LEGACY_SWAP_BEHAVIOUR_REQUIRES_COPY_CTOR) && FERMAT_DEQUE_LEGACY_SWAP_BEHAVIOUR_REQUIRES_COPY_CTOR
		...
		// usage of deque with non-copyable types (eg. fermat::deque<non_copyable> or fermat::deque<unique_ptr>).
		//
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	template<typename Integer>
	void deque<T, Alignment, Allocator, kDequeSubarraySize>::DoInit(Integer n, Integer value, std::true_type) {
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	template<typename InputIterator>
	void deque<T, Alignment, Allocator, kDequeSubarraySize>::DoInit(InputIterator first, InputIterator last, std::false_type) {
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	template<typename InputIterator>
	void deque<T, Alignment, Allocator, kDequeSubarraySize>::DoInitFromIterator(InputIterator first, InputIterator last,
	                                                                 std::input_iterator_tag) {
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	template<typename ForwardIterator>
	void deque<T, Alignment, Allocator, kDequeSubarraySize>::DoInitFromIterator(ForwardIterator first, ForwardIterator last,
		...
		std::uninitialized_copy((non_const_iterator_type) first, (non_const_iterator_type) last,
			...
			for (iterator itCurrent(mItBegin), itEnd(pPtrArrayCurrent, *pPtrArrayCurrent); itCurrent != itEnd; ++
			     itCurrent)
				itCurrent.mpCurrent->~value_type();
			throw;
		}
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	void deque<T, Alignment, Allocator, kDequeSubarraySize>::DoFillInit(const value_type &value) {
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	template<typename Integer>
	void deque<T, Alignment, Allocator, kDequeSubarraySize>::DoAssign(Integer n, Integer value, std::true_type)
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	template<typename InputIterator>
	void deque<T, Alignment, Allocator, kDequeSubarraySize>::DoAssign(InputIterator first, InputIterator last, std::false_type)
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	void deque<T, Alignment, Allocator, kDequeSubarraySize>::DoAssignValues(size_type n, const value_type &value) {
		const size_type nSize = size();

		if (n > nSize) // If we are increasing the size...
		{
			fermat::fill(mItBegin, mItEnd, value);
	...
	template<typename Integer>
	typename deque<T, Alignment, Allocator, kDequeSubarraySize>::iterator
	deque<T, Alignment, Allocator, kDequeSubarraySize>::DoInsert(const const_iterator &position, Integer n, Integer value,
	                                                  std::true_type) {
	...
	template<typename InputIterator>
	typename deque<T, Alignment, Allocator, kDequeSubarraySize>::iterator
	deque<T, Alignment, Allocator, kDequeSubarraySize>::DoInsert(const const_iterator &position, const InputIterator &first,
	...
	template<typename InputIterator>
	typename deque<T, Alignment, Allocator, kDequeSubarraySize>::iterator
	deque<T, Alignment, Allocator, kDequeSubarraySize>::DoInsertFromIterator(const_iterator position, const InputIterator &first,
	...
	template<typename ForwardIterator>
	typename deque<T, Alignment, Allocator, kDequeSubarraySize>::iterator
	deque<T, Alignment, Allocator, kDequeSubarraySize>::DoInsertFromIterator(const_iterator position, const ForwardIterator &first,
		...
		if (position.mpCurrent == mItBegin.mpCurrent) // If inserting at the beginning or into an empty container...
		{
			...
			return mItBegin;
		} else if (FERMAT_UNLIKELY(position.mpCurrent == mItEnd.mpCurrent))
		// If inserting at the end (i.e. appending)...
		{
			...
			// If the insertion index is in the front half of the deque... grow the deque at the front.
			{
				...
				} else // Else the newly inserted items are going within the newly allocated area at the front.
				{
					...
					fermat::uninitialized_copy_copy(mItBegin, itPosition, first, mid, itNewBegin); // This can throw.
				...
				if (nPushedCount > (difference_type) n) {
					...
					fermat::uninitialized_copy_copy(mid, last, itPosition, mItEnd, mItEnd);
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	typename deque<T, Alignment, Allocator, kDequeSubarraySize>::iterator
	deque<T, Alignment, Allocator, kDequeSubarraySize>::DoInsertValues(const_iterator position, size_type n,
		...
		// If inserting at the end (i.e. appending)...
		{
			...
			// If the insertion index is in the front half of the deque... grow the deque at the front.
			{
				...
				// If the newly inserted items will be entirely within the old area...
				{
					...
					// Recycle 'itUCopyEnd' to mean something else.
					fermat::fill(itUCopyEnd, itPosition, valueSaved);
				} else // Else the newly inserted items are going within the newly allocated area at the front.
				{
					std::uninitialized_fill(mItBegin, itPosition, itNewBegin, mItBegin, valueSaved);
					// This can throw.
					fermat::fill(itOldBegin, itPosition, valueSaved);
				}
				mItBegin = itNewBegin;
#if FERMAT_EXCEPTIONS_ENABLED
					}
			...
			} else // Else the insertion index is in the back half of the deque, so grow the deque at the back.
			{
				...
				// If the newly inserted items will be entirely within the old area...
				{
					...
					// Recycle 'itUCopyEnd' to mean something else.
					fermat::fill(itPosition, itUCopyEnd, valueSaved);
				} else // Else the newly inserted items are going within the newly allocated area at the back.
				{
					std::uninitialized_fill(mItEnd, itPosition + (difference_type) n, valueSaved, itPosition,
					                                mItEnd); // This can throw.
					fermat::fill(itPosition, itOldEnd, valueSaved);
				}
				mItEnd = itNewEnd;
#if FERMAT_EXCEPTIONS_ENABLED
					}
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	inline void deque<T, Alignment, Allocator, kDequeSubarraySize>::DoSwap(this_type &x) {
		...
		std::swap(mAllocator, x.mAllocator); // We do this even if FERMAT_ALLOCATOR_COPY_ENABLED is 0.
	}


	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	inline bool deque<T, Alignment, Allocator, kDequeSubarraySize>::validate() const {
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	inline bool operator==(const deque<T, Alignment, Allocator, kDequeSubarraySize> &a,
	                       const deque<T, Alignment, Allocator, kDequeSubarraySize> &b) {
		return ((a.size() == b.size()) && std::equal(a.begin(), a.end(), b.begin()));
	}

#if defined(KM_COMPILER_HAS_THREE_WAY_COMPARISON)
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	inline synth_three_way_result<T> operator<=>(const deque<T, Alignment, Allocator, kDequeSubarraySize> &a, const deque<T,
	...
	template<typename T, size_t Alignment, typename Allocator, unsigned kDequeSubarraySize>
	inline void swap(deque<T, Alignment, Allocator, kDequeSubarraySize> &a, deque<T, Alignment, Allocator, kDequeSubarraySize> &b) {
		a.swap(b);
	}
	...
	///////////////////////////////////////////////////////////////////////
	template<typename T,size_t Alignment,  typename Allocator, class U>
	typename deque<T, Alignment, Allocator>::size_type erase(deque<T, Alignment, Allocator> &c, const U &value) {
		...
		return static_cast<typename deque<T, Alignment, Allocator>::size_type>(numRemoved);
	}

	template<typename T,size_t Alignment,  typename Allocator, class Predicate>
	typename deque<T, Alignment, Allocator>::size_type erase_if(deque<T, Alignment, Allocator> &c, Predicate predicate) {
	...
	//
	///////////////////////////////////////////////////////////////////////
	template<typename T,size_t Alignment,  typename Allocator, unsigned SubArraySize, class U>
	typename deque<T, Alignment, Allocator, SubArraySize>::size_type erase_unsorted(
		deque<T, Alignment, Allocator, SubArraySize> &c, const U &value) {
	...
	//
	///////////////////////////////////////////////////////////////////////
	template<class T, size_t Alignment, class Allocator, class Predicate, unsigned SubArraySize>
	typename deque<T, Alignment, Allocator, SubArraySize>::size_type erase_unsorted_if(
		deque<T, Alignment, Allocator, SubArraySize> &c, Predicate predicate) {
