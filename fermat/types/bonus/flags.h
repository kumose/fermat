...
// required on the user's part. If the user had accidentally used fermat::maskflags<>
// instead of bitflags<>, then no shifting would occur and there would be _bugs_:
// 
//    enum class Knights : uint16_t { Armoured, Cowardly, Shining, Ni };
//    using KnightFlags = fermat::bitflags<Knights>;
// 
...
// FERMAT_DECLARE_BITFLAGS or FERMAT_DECLARE_MASKFLAGS, or it can be defined
...
// 
// 
// FERMAT_DECLARE_BITFLAGS
// FERMAT_DECLARE_MASKFLAGS
// -------------------------
...
// 
//    FERMAT_DECLARE_BITFLAGS(DragonFlags, DragonType);
// 
//      ^ the above macro has expanded to something akin to:
// 
//        using DragonFlags = ::fermat::bitflags<DragonType>;
...
//    };
// 
//    FERMAT_DECLARE_MASKFLAGS(WizardFlags, WizardType);
// 
//      ^ macro expansion for the above macro looks like:
// 
//        using WizardFlags = ::fermat::maskflags<WizardType>;
...
// 
// 
// FERMAT_DECLARE_BITFLAGS_ENUM_CLASS
// FERMAT_DECLARE_MASKFLAGS_ENUM_CLASS
// -----------------------------------
...
// 
//    FERMAT_DECLARE_BITFLAGS_ENUM_CLASS(DragonFlags, DragonType)
//    {
//        Jubilent,
//        Depressed,
//        Smashed
...
// without the macro:
// 
//    FERMAT_DECLARE_MASKFLAGS_ENUM_CLASS(SquireFlags, Squires)
//    {
//        Absent = 0,
//        Inexperienced = (1<<0),
//        Keen =          (1<<1),
...
// 
//    enum class Squires;
//    using SquireFlags = ::fermat::bitflags<Squires>;
...
namespace fermat::detail {
    template<size_t N>
    struct flags_mask_type;

    template<>
    struct flags_mask_type<1> {
        using type = uint8_t;
    };

    template<>
    struct flags_mask_type<2> {
        using type = uint16_t;
    };

    template<>
    struct flags_mask_type<4> {
        using type = uint32_t;
    };

    template<>
    struct flags_mask_type<8> {
        using type = uint64_t;
    };
...
namespace fermat {
    ...
    struct basic_flags {
        ...
        static_assert(std::is_integral_v<mask_type>, "mask_type must be integral");
    ...
    private:
        ...
        friend constexpr basic_flags<Y, U> operator &(basic_flags<Y, U>, basic_flags<Y, U>) noexcept;
        ...
        template<typename Y, typename U>
        friend constexpr basic_flags<Y, U> operator
        &(basic_flags<Y, U>, typename basic_flags<Y, U>::flag_type) noexcept;
        ...
        template<typename Y, typename U>
        friend constexpr basic_flags<Y, U> operator
        &(typename basic_flags<Y, U>::flag_type, basic_flags<Y, U>) noexcept;
        ...
        template<typename Y, typename U>
        friend constexpr bool operator ==(basic_flags<Y, U>, basic_flags<Y, U>) noexcept;

        template<typename Y, typename U>
        friend constexpr bool operator ==(basic_flags<Y, U>, typename basic_flags<Y, U>::flag_type) noexcept;

        template<typename Y, typename U>
        friend constexpr bool operator ==(typename basic_flags<Y, U>::flag_type, basic_flags<Y, U>) noexcept;
...
namespace fermat {
    ...
    template<typename F, typename M>
    inline constexpr basic_flags<F, M> operator &(basic_flags<F, M> lhs,
                                                  typename basic_flags<F, M>::flag_type rhs) noexcept {
    ...
    template<typename F, typename M>
    inline constexpr basic_flags<F, M> operator &(typename basic_flags<F, M>::flag_type lhs,
                                                  basic_flags<F, M> rhs) noexcept {
...
//
#define FERMAT_DECLARE_BITFLAGS(flagstype, enumtype) \
	using flagstype = ::fermat::bitflags<enumtype>; \
...
#define FERMAT_DECLARE_BITFLAGS_ENUM_CLASS(flagstype, enumtype) \
	enum class enumtype; \
	FERMAT_DECLARE_BITFLAGS(flagstype, enumtype) \
	enum class enumtype

#define FERMAT_DECLARE_BITFLAGS_ENUM_CLASS_SIZED(sizetype, flagstype, enumtype) \
	enum class enumtype : sizetype; \
	FERMAT_DECLARE_BITFLAGS(flagstype, enumtype) \
	enum class enumtype : sizetype


#define FERMAT_DECLARE_MASKFLAGS(flagstype, enumtype) \
	using flagstype = ::fermat::maskflags<enumtype>; \
	inline constexpr flagstype operator | (enumtype lhs, enumtype rhs) \
	{ \
		using msh = typename flagstype::marshaller_type; \
		return flagstype::from_mask(static_cast<msh::mask_type>(msh::to_mask(lhs) | msh::to_mask(rhs))); \
...
#define FERMAT_DECLARE_MASKFLAGS_ENUM_CLASS(flagstype, enumtype) \
	enum class enumtype; \
	FERMAT_DECLARE_MASKFLAGS(flagstype, enumtype) \
	enum class enumtype

#define FERMAT_DECLARE_MASKFLAGS_ENUM_CLASS_SIZED(sizetype, flagstype, enumtype) \
	enum class enumtype : sizetype; \
	FERMAT_DECLARE_MASKFLAGS(flagstype, enumtype) \
	enum class enumtype : sizetype
