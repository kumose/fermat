...
#ifndef FERMAT_ADAPTORS_H
#define FERMAT_ADAPTORS_H
...
KM_DISABLE_VC_WARNING (5027) // move assignment operator was implicitly defined as deleted
#endif
...
namespace fermat {
    ...
    struct reverse_wrapper {
        ...
        reverse_wrapper(C &&c)
			 ...
			 */
            static_assert(std::is_same_v<C, Container>,
    ...
    template<typename Container>
    auto begin(const reverse_wrapper<Container> &w) -> decltype(fermat::rbegin(w.mContainer)) {
        return fermat::rbegin(w.mContainer);
    }

    template<typename Container>
    auto end(const reverse_wrapper<Container> &w) -> decltype(fermat::rend(w.mContainer)) {
        return fermat::rend(w.mContainer);
    }
