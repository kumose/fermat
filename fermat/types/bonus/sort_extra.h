...
#include <fermat/types/functional.h>
#include <fermat/types/heap.h>
#include <fermat/types/sort.h>             // For backwards compatibility due to sorts moved from here to sort.h.
...
namespace fermat {
        ...
        for (; first != last; ++first) {
            ...
            for (++iCurrent; iCurrent != last; ++iCurrent) {
                if (compare(*iCurrent, *iMin)) {
                    FERMAT_VALIDATE_COMPARE(!compare(*iMin, *iCurrent)); // Validate that the compare function is sane.
                    iMin = iCurrent;
                }
            }
    ...
    inline void selection_sort(ForwardIterator first, ForwardIterator last) {
        ...
        fermat::selection_sort<ForwardIterator, Less>(first, last, Less());
    }
                ...
                if (first != last) {
                    for (iCurrent = last; iCurrent != first; iCurrent = iNext) {
                        ...
                        if (compare(*iCurrent, *iNext)) {
                            FERMAT_VALIDATE_COMPARE(!compare(*iNext, *iCurrent));
                            // Validate that the compare function is sane.
                            iLastModified = iCurrent;
    ...
    inline void shaker_sort(BidirectionalIterator first, BidirectionalIterator last) {
        ...
        fermat::shaker_sort<BidirectionalIterator, Less>(first, last, Less());
    }
	...
	///  bucket_sort(intArray.begin(), intArray.end(), bucketArray, fermat::hash_use_self<int>());
	///
    template<typename T>
    ...
    void bucket_sort(ForwardIterator first, ForwardIterator last, ContainerArray &bucketArray,
                     HashFunction hash /*= hash_use_self*/) {
        for (ForwardIterator iInput = first; iInput != last; ++iInput)
            bucketArray[hash(*iInput)].push_back(*iInput);

        for (typename ContainerArray::const_iterator iBucket = bucketArray.begin(); iBucket != bucketArray.end(); ++
             iBucket)
