...
namespace fermat {
    ...
    public:
        ...
        explicit IntrusiveSDListIterator(pointer pNode);

        // Note that you can also construct an iterator from T via this, since value_type == node_type.
    ...
    public:
        ...
        ///< Moves the given element into this list before the element pointed to by position; O(1).
																		 ///< Required: x must be in some list or have first/next pointers that point it itself.

        void splice(iterator position, this_type &x);

        ///< Moves the contents of a list into this list before the element pointed to by position; O(1).
    ...
    inline intrusive_sdlist_base::intrusive_sdlist_base() { mpNext = NULL; }


    inline bool intrusive_sdlist_base::empty() const { return mpNext == NULL; }
    ...
    inline void intrusive_sdlist_base::clear() { mpNext = NULL; } // Note that we don't do anything with the list nodes.
    ...
    template<typename T>
    inline typename intrusive_sdlist<T>::reference intrusive_sdlist<T>::front() { return *static_cast<T *>(mpNext); }
    ...
    void intrusive_sdlist<T>::splice(iterator /*position*/, value_type & /*value*/) {
        FERMAT_ASSERT(false); // If you need this working, ask Paul Pedriana or submit a working version for inclusion.
    }
    ...
    template<typename T>
    void intrusive_sdlist<T>::splice(iterator /*position*/, intrusive_sdlist & /*x*/, iterator /*first*/,
                                     iterator /*last*/) {
        FERMAT_ASSERT(false); // If you need this working, ask Paul Pedriana or submit a working version for inclusion.
    }
