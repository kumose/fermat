...
namespace fermat {
    ...
    uniform_int_distribution<IntType>::operator()(Generator &g, const param_type &params) {
        ...
        static_assert(std::is_unsigned<result_type>::value && (sizeof(result_type) <= 4),
                      "uniform_int_distribution currently supports only uint8_t, uint16_t, uint32_t.");

        result_type v = g();
        // Generates a value in the range of (numeric_limits<result_type>::min(), numeric_limits<result_type>::max()).
        result_type r = (result_type) ((v * (uint64_t) ((params.b() - params.a()) + 1)) >> (sizeof(result_type) * 8));
    ...
    // fermat::basic_ostream<CharT, Traits>& operator<<(fermat::basic_ostream<CharT, Traits>& os, const uniform_int_distribution& uid);
    //
    // template<class CharT, class Traits, class ResultType>
    // fermat::basic_istream<CharT, Traits>& operator>>(fermat::basic_istream<CharT, Traits>& is, uniform_int_distribution& uid);
} // namespace fermat
