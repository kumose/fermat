...
namespace fermat {
	...
	template<typename T, size_t Count, size_t Alignment, typename Allocator = BasicAllocator<T, Alignment>>
	class segment {
	public:
		typedef size_t size_type;
		typedef segment<T, Count,  Alignment, Allocator> this_type;
		typedef T *iterator;
	...
	private:
		...
		template<typename, size_t,size_t, typename>
		friend struct segmented_vector_iterator;
	};
	...
	public:
		typedef segmented_vector_iterator<T, Count,  Alignment, Allocator> this_type;
		typedef segment<T, Count,  Alignment, Allocator> segment_type;
	...
	public:
		typedef size_t size_type;
		typedef segmented_vector<T, Count,  Alignment, Allocator> this_type;
		typedef segment<T, Count, Alignment, Allocator> segment_type;
		typedef Allocator allocator_type;
		typedef segmented_vector_iterator<const T, Count,  Alignment, Allocator> const_iterator;
		typedef segmented_vector_iterator<T, Count,  Alignment, Allocator> iterator;
	...
	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline const segment<T, Count, Alignment,  Allocator> *
	segment<T, Count, Alignment,  Allocator>::next_segment() const {
	...
	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline segment<T, Count, Alignment,  Allocator> *
	segment<T, Count, Alignment,  Allocator>::next_segment() {
		if (mPrev & kIsLastSegment)
			return nullptr;
		else
			return mNext;
	...
	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline typename segment<T, Count, Alignment,  Allocator>::const_iterator
	segment<T, Count, Alignment,  Allocator>::begin() const {
		return reinterpret_cast<const T *>(&mData);
	}

	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline typename segment<T, Count, Alignment,  Allocator>::iterator
	segment<T, Count, Alignment,  Allocator>::begin() {
		return reinterpret_cast<T *>(&mData);
	}

	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline typename segment<T, Count, Alignment,  Allocator>::const_iterator
	segment<T, Count, Alignment,  Allocator>::end() const {
		if (mPrev & kIsLastSegment)
			return begin() + mSize;
		else
			return begin() + Count;
	...
	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline typename segment<T, Count, Alignment,  Allocator>::iterator
	segment<T, Count, Alignment,  Allocator>::end() {
		if (mPrev & kIsLastSegment)
			return begin() + mSize;
		else
			return begin() + Count;
	...
	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline void segment<T, Count, Alignment,  Allocator>::DestroyData() {
	...
	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	T *
	segmented_vector_iterator<T, Count, Alignment,  Allocator>::operator->() const {
		return mCurrent;
	}
	...
	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	segmented_vector_iterator<T, Count, Alignment,  Allocator> &
	segmented_vector_iterator<T, Count, Alignment,  Allocator>::operator++() {
		++mCurrent;
		if (FERMAT_UNLIKELY(mCurrent == mEnd)) {
	...
	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	segmented_vector_iterator<T, Count, Alignment,  Allocator>
	segmented_vector_iterator<T, Count, Alignment,  Allocator>::operator++(int) {
		this_type i(*this);
		operator++();
		return i;
	}
	...
	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline segmented_vector<T, Count, Alignment,  Allocator>::segmented_vector(const Allocator &allocator)
		: mAllocator(allocator) {
	}

	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline segmented_vector<T, Count, Alignment,  Allocator>::segmented_vector(const segmented_vector &other)
		: mAllocator(other.mAllocator) {
		InsertRange<false>(other.begin(), other.end());
	}

	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline segmented_vector<T, Count, Alignment,  Allocator>::segmented_vector(segmented_vector &&other)
		: mAllocator(other.mAllocator) {
		swap(other);
	}

	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline segmented_vector<T, Count, Alignment,  Allocator>::segmented_vector(std::initializer_list<value_type> ilist,
	...
	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline segmented_vector<T, Count, Alignment,  Allocator> &segmented_vector<T, Count, Alignment,  Allocator>::operator=(
		const segmented_vector &other) {
		if (KM_UNLIKELY(this == &other)) {
			return *this;
		}
	...
	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline segmented_vector<T, Count, Alignment,  Allocator>::~segmented_vector() {
		Clear<true>();
	}

	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline const typename segmented_vector<T, Count, Alignment,  Allocator>::allocator_type &
	segmented_vector<T, Count, Alignment,  Allocator>::get_allocator() const noexcept {
		return mAllocator;
	}

	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline typename segmented_vector<T, Count, Alignment,  Allocator>::allocator_type &
	segmented_vector<T, Count, Alignment,  Allocator>::get_allocator() noexcept {
		return mAllocator;
	}

	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline const typename segmented_vector<T, Count, Alignment,  Allocator>::segment_type *
	segmented_vector<T, Count, Alignment,  Allocator>::first_segment() const noexcept {
		return mFirstSegment;
	}

	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline typename segmented_vector<T, Count, Alignment,  Allocator>::segment_type *
	segmented_vector<T, Count, Alignment,  Allocator>::first_segment() noexcept {
		return mFirstSegment;
	}

	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline typename segmented_vector<T, Count, Alignment,  Allocator>::const_iterator
	segmented_vector<T, Count, Alignment,  Allocator>::begin() const noexcept {
		iterator i;
	...
	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline typename segmented_vector<T, Count, Alignment,  Allocator>::iterator
	segmented_vector<T, Count, Alignment,  Allocator>::begin() noexcept {
		iterator i;
		i.mSegment = mFirstSegment;
		if (mFirstSegment) {
			i.mCurrent = mFirstSegment->begin();
	...
	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline typename segmented_vector<T, Count, Alignment,  Allocator>::const_iterator
	segmented_vector<T, Count, Alignment,  Allocator>::end() const noexcept {
		iterator i;
		i.mCurrent = nullptr;
		return (const_iterator &) i;
	}
	...
	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline typename segmented_vector<T, Count, Alignment,  Allocator>::iterator
	segmented_vector<T, Count, Alignment,  Allocator>::end() noexcept {
		iterator i;
		i.mCurrent = nullptr;
		return i;
	}
	...
	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline typename segmented_vector<T, Count, Alignment,  Allocator>::size_type
	segmented_vector<T, Count, Alignment,  Allocator>::size() const noexcept {
	...
	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline typename segmented_vector<T, Count, Alignment,  Allocator>::size_type
	segmented_vector<T, Count, Alignment,  Allocator>::capacity() const noexcept {
		return (mInUseSegmentCount + mFreeListSegmentCount) * Count;
	}

	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline typename segmented_vector<T, Count, Alignment,  Allocator>::size_type
	segmented_vector<T, Count, Alignment,  Allocator>::segment_count() const noexcept {
		return mInUseSegmentCount;
	}

	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline T &
	segmented_vector<T, Count, Alignment,  Allocator>::front() noexcept {
#if FERMAT_ASSERT_ENABLED && FERMAT_EMPTY_REFERENCE_ASSERT_ENABLED
		if (FERMAT_UNLIKELY(mFirstSegment == nullptr)) // We don't allow the user to reference an empty container.
			FERMAT_FAIL_MSG("segmented_vector::front -- empty container");
#endif
	...
	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline T &
	segmented_vector<T, Count, Alignment,  Allocator>::back() noexcept {
#if FERMAT_ASSERT_ENABLED && FERMAT_EMPTY_REFERENCE_ASSERT_ENABLED
		if (FERMAT_UNLIKELY(mLastSegment == nullptr)) // We don't allow the user to reference an empty container.
			FERMAT_FAIL_MSG("segmented_vector::back -- empty container");
#endif
	...
	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline bool
	segmented_vector<T, Count, Alignment,  Allocator>::empty() const noexcept {
		return mFirstSegment == nullptr;
	}

	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline void
	segmented_vector<T, Count, Alignment,  Allocator>::clear() {
		Clear<false>();
	}

	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline void segmented_vector<T, Count, Alignment,  Allocator>::reserve(size_type n) {
		while (capacity() < n) {
	...
	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline void segmented_vector<T, Count, Alignment,  Allocator>::resize(size_type n, const value_type &v) {
		if (size() < n) {
			PushBack(n - size(), v);
		} else {
	...
	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline void
	segmented_vector<T, Count, Alignment,  Allocator>::resize(size_type n) {
		resize(n, value_type());
	}

	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline void segmented_vector<T, Count, Alignment,  Allocator>::shrink_to_fit() noexcept {
		static_assert(std::is_trivially_destructible_v<segment_type>,
	...
	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline T &
	segmented_vector<T, Count, Alignment,  Allocator>::push_back() {
		return *(::new(DoPushBack()) T());
	}

	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline T &
	segmented_vector<T, Count, Alignment,  Allocator>::push_back(const T &value) {
		return *(::new(DoPushBack()) T(value));
	}

	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline T &
	segmented_vector<T, Count, Alignment,  Allocator>::push_back(T &&value) {
		return *(::new(DoPushBack()) T(std::move(value)));
	}

	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline void *
	segmented_vector<T, Count, Alignment,  Allocator>::push_back_uninitialized() {
		return DoPushBack();
	}

	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	template<class... Args>
	T &segmented_vector<T, Count, Alignment,  Allocator>::emplace_back(Args &&... args) {
		return *(::new(DoPushBack()) T(std::forward<Args>(args)...));
	}

	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline void
	segmented_vector<T, Count, Alignment,  Allocator>::pop_back() {
		segment_type *lastSegment = mLastSegment;
#if FERMAT_ASSERT_ENABLED
		if (FERMAT_UNLIKELY(!lastSegment))
			FERMAT_FAIL_MSG("segmented_vector::pop_back -- segmented vector is empty");
#endif
		--lastSegment->mSize;
		T *const toDestroy = lastSegment->begin() + lastSegment->mSize;
		std::destroy_at(toDestroy);
	...
	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline void
	segmented_vector<T, Count, Alignment,  Allocator>::erase_unsorted(segment_type &segment, typename segment_type::iterator it) {
		KM_UNUSED(segment);
	...
	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline typename segmented_vector<T, Count, Alignment,  Allocator>::iterator
	segmented_vector<T, Count, Alignment,  Allocator>::erase_unsorted(const iterator &i) {
		iterator ret(i);
		*i = back();
	...
	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	void
	segmented_vector<T, Count, Alignment,  Allocator>::swap(this_type &other) {
		using std::swap;
		//
	...
	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline void *
	segmented_vector<T, Count, Alignment,  Allocator>::DoPushBack() {
	...
	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline void
	segmented_vector<T, Count, Alignment,  Allocator>::AddToFreeList(segment_type *segment) {
	...
	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline typename segmented_vector<T, Count, Alignment,  Allocator>::segment_type *
	segmented_vector<T, Count, Alignment,  Allocator>::GetUnusedSegmentForLastSegment(segment_type *prevSegment) {
	...
	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline typename segmented_vector<T, Count, Alignment,  Allocator>::segment_type *
	segmented_vector<T, Count, Alignment,  Allocator>::AllocateNewSegment() {
		static_assert(std::is_trivially_constructible_v<segment_type>, "We're not initializing segment_type here");
		return (segment_type *) allocate_memory(mAllocator, sizeof(segment_type), FERMAT_ALIGN_OF(segment_type), 0);
	}

	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	template<bool bFreeMemory>
	inline void segmented_vector<T, Count, Alignment,  Allocator>::Clear() {
		if (bFreeMemory) {
	...
	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline void segmented_vector<T, Count, Alignment,  Allocator>::UpdateLastSegment() {
	...
	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline void segmented_vector<T, Count, Alignment,  Allocator>::EraseFromBack(size_type toRemoveCount) {
		...
		for (size_type i = 1; i <= toRemoveCount; ++i) {
			...
			std::destroy_at(toRemove);
		}
		mLastSegment->mSize -= toRemoveCount;
	}

	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline void segmented_vector<T, Count, Alignment,  Allocator>::PushBack(size_type toAddCount, const value_type &v) {
		...
		const auto &fillLastSegment = [&]() {
			...
			const size_type addedThisLoop = std::min(toAddCount, spaceInSegment);
	...
	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	template<bool bDoMove, typename ForwardIt>
	inline void segmented_vector<T, Count, Alignment,  Allocator>::InsertRange(ForwardIt begin, ForwardIt end) {
	...
	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline bool operator==(const segmented_vector_iterator<const T, Count, Alignment,  Allocator> &a,
	                       const segmented_vector_iterator<const T, Count, Alignment,  Allocator> &b) {
	...
	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline bool operator==(const segmented_vector_iterator<T, Count, Alignment,  Allocator> &a,
	                       const segmented_vector_iterator<T, Count, Alignment,  Allocator> &b) {
		return a.mCurrent == b.mCurrent;
	}
	...
	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline bool operator==(const segmented_vector<T, Count, Alignment,  Allocator> &a,
	                       const segmented_vector<T, Count, Alignment,  Allocator> &b) {
		return (a.size() == b.size()) && std::equal(a.begin(), a.end(), b.begin());
	}

#if defined(KM_COMPILER_HAS_THREE_WAY_COMPARISON)
	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline synth_three_way_result<T> operator<=>(const segmented_vector<T, Count, Alignment,  Allocator> &a, const segmented_vector<
		T, Count, Alignment,  Allocator> &b)
	{
	...
	template<typename T, size_t Count, size_t Alignment,  typename Allocator>
	inline void swap(segmented_vector<T, Count, Alignment,  Allocator> &a, segmented_vector<T, Count, Alignment,  Allocator> &b) {
		a.swap(b);
	}
}
