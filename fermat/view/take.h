...
namespace fermat::ranges {
    ...
    private:
        ...
        static auto begin_sized_(Take &take, std::false_type) {
            return make_counted_iterator(fermat::ranges::begin(take.base_), take.count_);
        }
        ...
        static auto end_sized_(take_view &take, std::false_type, std::false_type) {
            return sentinel<false>{fermat::ranges::end(take.base_)};
        }
        static auto end_sized_(take_view const &take, std::false_type, std::false_type) {
            return sentinel<true>{fermat::ranges::end(take.base_)};
        }
#endif
    ...
    public:
        ...
        constexpr auto CPP_fun(begin)()(
            ...
            if constexpr (sized_range<Rng>)
                if constexpr (random_access_range<Rng>)
                    return fermat::ranges::begin(base_);
                else {
                    // cannot always delegate to size() member on GCC with ConceptsTS
#if defined(__cpp_concepts) && __cpp_concepts <= 201507
                    auto s = fermat::ranges::min(
                        static_cast<range_difference_t<Rng>>(count_),
                        static_cast<range_difference_t<Rng>>(fermat::ranges::size(base_)));
#else
                    auto s = static_cast<range_difference_t<Rng>>(size());
#endif
                    return make_counted_iterator(fermat::ranges::begin(base_), s);
                }
            else
                return make_counted_iterator(fermat::ranges::begin(base_), count_);
#else
        ...
        constexpr auto CPP_fun(end)()(
            ...
            if constexpr (sized_range<Rng>)
                ...
                return sentinel<false>{fermat::ranges::end(base_)};
#else
        ...
        constexpr auto CPP_fun(end)()(const //
            ...
            if constexpr (sized_range<Rng const>)
                ...
                return sentinel<true>{fermat::ranges::end(base_)};
#else
            return end_sized_(
        ...
        constexpr auto CPP_fun(size)()(
            ...
            auto n = fermat::ranges::size(base_);
            return fermat::ranges::min(n, static_cast<decltype(n)>(count_));
        }
    ...
    namespace cpp20 {
        namespace views {
            using fermat::ranges::views::take;
        }

        template(typename Rng)(
            requires view_<Rng>)
        using take_view = fermat::ranges::take_view<Rng>;
    } // namespace cpp20
    /// @}
} // namespace fermat::ranges
...
RANGES_SATISFY_BOOST_RANGE(::fermat::ranges::take_view)
