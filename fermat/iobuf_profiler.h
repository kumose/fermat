...
struct IOBufSample {
    ...
    static IOBufSample* New() {
        return turbo::get_object<IOBufSample>();
    }
    ...
    static void Destroy(IOBufSample* ref) {
        ref->_hash_code = 0;
        turbo::return_object(ref);
    }

    size_t stack_hash_code() const;

private:
friend turbo::ObjectPool<IOBufSample>;
...
static_assert(sizeof(IOBufSample) == 256, "be_friendly_to_allocator");
...
class IOBufProfiler : public turbo::SimpleThread {
public:
    ...
    void StopAndJoin();
    static  IOBufProfiler& instance() {
        static IOBufProfiler instance;
        return instance;
    }
private:

    typedef turbo::flat_hash_map<IOBufSample*,
    ...
    typedef turbo::flat_hash_map<IOBufRefSampleSharedPtr,
                           int64_t,
    ...
    typedef turbo::flat_hash_map<CordBuffer::Block*, BlockInfo> BlockInfoMap;

    IOBufProfiler();
    ~IOBufProfiler() override;
    TURBO_DISALLOW_COPY_AND_ASSIGN(IOBufProfiler);
    ...
    // IOBuf sample queue.
    turbo::MPSCQueue<IOBufSample*> _sample_queue;

    // Temp buf before saving the file.
    CordBuffer _disk_buf;
