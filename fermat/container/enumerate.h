    ...
    namespace detail {
        ...
        public:
            ...
            public:
                ...
                TURBO_FORCE_INLINE constexpr explicit Proxy(const Enumerator &e)
                    : index(e.idx_), element(*e.it_) {
                }

                // Non-const Proxy: Forward constness from Iterator.
                TURBO_FORCE_INLINE constexpr reference operator*() { return element; }
                TURBO_FORCE_INLINE constexpr pointer operator->() {
                    return std::addressof(element);
                }

                // Const Proxy: Force const references.
                TURBO_FORCE_INLINE constexpr typename MakeConst<reference>::type
                operator*() const {
                    return element;
                }

                TURBO_FORCE_INLINE constexpr typename MakeConst<pointer>::type operator->()
                const {
                    return std::addressof(element);
                }
            ...
            TURBO_FORCE_INLINE constexpr Proxy operator*() const { return Proxy(*this); }

            TURBO_FORCE_INLINE constexpr Enumerator &operator++() {
                ++it_;
                ++idx_;
                return *this;
            }
            ...
            TURBO_FORCE_INLINE constexpr bool operator==(
