...
namespace fermat {
	...
	template<typename Key,
		typename Value,
		typename list_type = std::list<Key>,
		typename map_type = turbo::flat_hash_map<Key,
			std::pair<Value, typename list_type::iterator>> >
	class LruCache {
	public:
		using key_type = Key;
		...
		using data_container_type = std::pair<value_type, list_iterator>;
		using iterator = typename map_type::iterator;
		using const_iterator = typename map_type::const_iterator;
		using this_type = LruCache<key_type, value_type, list_type, map_type>;
		using create_callback_type = std::function<value_type(key_type)>;
		using delete_callback_type = std::function<void(const value_type &)>;
		...
		explicit LruCache(size_type size,
		...
		LruCache(std::initializer_list<std::pair<Key, Value> > il)
			: LruCache(static_cast<size_type>(il.size())) {
			for (auto &p: il)
				insert_or_assign(p.first, p.second);
		}

		LruCache(const this_type &) = delete;
		...
		iterator rbegin() noexcept { return m_map.rbegin(); }
		iterator rend() noexcept { return m_map.rend(); }
		const_iterator begin() const noexcept { return m_map.begin(); }
		const_iterator cbegin() const noexcept { return m_map.cbegin(); }
		const_iterator crbegin() const noexcept { return m_map.crbegin(); }
