...
#define BITSET_WORD_COUNT(nBitCount, WordType) ((nBitCount - 1) / (8 * sizeof(WordType)) + 1)
    ...
    namespace detail {
        ...
        struct is_word_type : std::bool_constant<
                    !std::is_const_v<T> && !std::is_volatile_v<T> && !std::is_same_v<T, bool> && std::is_integral_v<
                        T> && std::is_unsigned_v<T>> {
        };
        ...
        std::enable_if_t<is_word_type_v<UInt> > from_unsigned_integral(Bitset<N, WordType> &bs, UInt value) {
            constexpr size_t numWords = (N > 0) ? ((N - 1) / (CHAR_BIT * sizeof(WordType)) + 1) : 0;
            // BITSET_WORD_COUNT(N, WordType) but 0 for N == 0

            WordType *data = bs.data();

            if constexpr (numWords > 0) {
                // copy everything from value into our word array:
                constexpr size_t bytes_to_copy = std::min(numWords * sizeof(WordType), sizeof(UInt));
                memcpy(data, &value, bytes_to_copy);

                // zero any remaining elements in our array:
                memset(reinterpret_cast<unsigned char *>(data) + bytes_to_copy, 0,
                       numWords * sizeof(WordType) - bytes_to_copy);
                ...
                if constexpr (N % (CHAR_BIT * sizeof(WordType)) != 0) {
        ...
        template<typename UInt, typename WordType, size_t NumWords, bool bAssertOnOverflow, bool bDoPartialCopy = (
            sizeof(UInt) < sizeof(WordType))>
        ...
        struct to_unsigned_integral_helper<UInt, WordType, NumWords, bAssertOnOverflow, true> {
            static size_t copyWords(const WordType *data, UInt *result) {
                ...
                constexpr WordType lastElemOverflowBitsMask = static_cast<WordType>(~(
                    (WordType(1) << (CHAR_BIT * sizeof(UInt))) - 1));
        ...
        std::enable_if_t<is_word_type_v<UInt>, UInt> to_unsigned_integral(const Bitset<N, WordType> &bs) {
            constexpr size_t kNumWords = (N > 0) ? ((N - 1) / (CHAR_BIT * sizeof(WordType)) + 1) : 0;
            // BITSET_WORD_COUNT(N, WordType) but 0 for N == 0
            ...
            if constexpr (kNumWords > 0) {
                ...
                const size_t numWordsCopied = to_unsigned_integral_helper<UInt, WordType, kNumWords,
                    bAssertOnOverflow>::copyWords(data, &result);
    ...
    // Templated on the number of words used to hold the Bitset and the word type.
    struct BitsetBase {
    ...
    public:
        ...
        enum {
            ...
            kWordCount = BITSET_WORD_COUNT(N, WordType)
            // The number of words the Bitset uses to hold the bits. sizeof(Bitset<N, WordType>) == kWordSize * kWordCount.
        };
    ...
    public:
        ...
        std::enable_if_t<detail::is_word_type_v<UInt> && N <= (CHAR_BIT * sizeof(UInt)), UInt>
        as_uint() const noexcept { return detail::to_unsigned_integral<UInt, true>(*this); }

        template<size_t NumBits = N>
        std::enable_if_t<NumBits <= (CHAR_BIT * sizeof(uint32_t)), uint32_t> as_uint32() const noexcept {
            return to_uint32_assert_convertible();
        }

        template<size_t NumBits = N>
        std::enable_if_t<NumBits <= (CHAR_BIT * sizeof(uint64_t)), uint64_t> as_uint64() const noexcept {
            return to_uint64_assert_convertible();
        }

        template<size_t NumBits = N>
        std::enable_if_t<NumBits <= (CHAR_BIT * sizeof(unsigned long)), unsigned long> as_ulong() const noexcept {
            return to_ulong_assert_convertible();
        }
    ...
    ///
    /// todo: Use bit.h's turbo::popcount instead?
    template<typename UInt64>
    std::enable_if_t<detail::is_word_type_v<UInt64> && sizeof(UInt64) == 8, uint32_t> BitsetCountBits(UInt64 x) {
    ...
    std::enable_if_t<detail::is_word_type_v<UInt32> && sizeof(UInt32) == 4, uint32_t> BitsetCountBits(UInt32 x) {
        x = x - ((x >> 1) & 0x55555555);
    ...
    std::enable_if_t<detail::is_word_type_v<SmallUInt> && sizeof(SmallUInt) < 4, uint32_t>
    BitsetCountBits(SmallUInt x) {
    ...
    BitsetBase<1, WordType>::DoFindPrev(size_type last_find) const {
        if (last_find > 0) {
            // Mask off previous bits of word so our search becomes a "find first".
            const word_type this_word =
                    mWord[0] & (static_cast<word_type>(~static_cast<word_type>(0)) >> (kBitsPerWord - last_find));
    ...
    inline void BitsetBase<2, WordType>::operator<<=(size_type n) {
        ...
        {
            if (TURBO_UNLIKELY(n >= kBitsPerWord)) // parent expected to handle high bits and n >= 64
            {
                mWord[1] = mWord[0];
                mWord[0] = 0;
                n -= kBitsPerWord;
    ...
    inline void BitsetBase<2, WordType>::operator>>=(size_type n) {
        if (n) // to avoid a shift by kBitsPerWord, which is undefined
        {
            if (TURBO_UNLIKELY(n >= kBitsPerWord)) // parent expected to handle n >= 64
            {
                mWord[0] = mWord[1];
                mWord[1] = 0;
                n -= kBitsPerWord;
    ...
    BitsetBase<2, WordType>::count() const {
        return turbo::popcount(mWord[0]) + turbo::popcount(mWord[1]);
    }
    ...
    BitsetBase<2, WordType>::DoFindPrev(size_type last_find) const {
        ...
        if (last_find > (size_type) kBitsPerWord) {
            ...
            // Mask off previous bits of word so our search becomes a "find first".
            word_type this_word = mWord[1] & (static_cast<word_type>(~static_cast<word_type>(0)) >> (
                                                  kBitsPerWord - last_find));
            ...
            // Mask off previous bits of word so our search becomes a "find first".
            word_type this_word = mWord[0] & (static_cast<word_type>(~static_cast<word_type>(0)) >> (
                                                  kBitsPerWord - last_find));
    ...
    Bitset<N, WordType>::operator<<=(size_type n) {
        if (TURBO_LIKELY((intptr_t) n < (intptr_t) N)) {
            base_type::operator<<=(n);
            if ((N & kBitsPerWordMask) || (N == 0))
                // If there are any high bits to clear... (If we didn't have this check, then the code below would do the wrong thing when N == 32.
    ...
    Bitset<N, WordType>::set() {
        ...
        if ((N & kBitsPerWordMask) || (N == 0))
            // If there are any high bits to clear... (If we didn't have this check, then the code below would do the wrong thing when N == 32.
            mWord[kWordCount - 1] &= ~(static_cast<word_type>(~static_cast<word_type>(0)) << (N & kBitsPerWordMask));
        // This clears any high unused bits. We need to do this so that shift operations proceed correctly.
        return *this;
    }
    ...
    Bitset<N, WordType>::set(size_type i, bool value) {
        if (i < N)
            ...
            if (TURBO_UNLIKELY(!(i < N)))
                KCHECK(false) << "Bitset::set -- out of range";
        }
    ...
    Bitset<N, WordType>::reset(size_type i) {
        if (TURBO_LIKELY(i < N))
            ...
            if (TURBO_UNLIKELY(!(i < N)))
                KCHECK(false) << "Bitset::reset -- out of range";
        }
    ...
    Bitset<N, WordType>::flip(size_type i) {
        if (TURBO_LIKELY(i < N))
            ...
            if (TURBO_UNLIKELY(!(i < N)))
                KCHECK(false) << "Bitset::flip -- out of range";
        }
        return *this;
    }
    ...
    Bitset<N, WordType>::operator[](size_type i) {
        if (TURBO_UNLIKELY(!(i < N)))
            KCHECK(false) << "Bitset::operator[] -- out of range";
    ...
    inline bool Bitset<N, WordType>::test(size_type i) const {
        ...
        KCHECK(false) << "Bitset::test -- out of range";
    }
