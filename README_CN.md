...
| `Buffer<T>` | 构造（所有规模）、小规模 push/迭代/中间插删、清除（Release） | 2x ~ 5x |
| `Vector<T>` | 与 `Buffer` 同路径优化；Release 下随机访问与 std 持平或更快 | 2x ~ 5x |
| `Deque<T, Alloc, kSubarray>` | 构造、两端 push、迭代、随机访问、中间插删、clear、析构 | 2x ~ 10x |
| `List<T>` | 所有操作 | 2x ~ 20x |
| `PriorityQueue<T>` | push（50–100k）、push/pop（≤2k）、迭代器构造、有界流 | 1.2x ~ 4x |
...
| 支持优先级修改的优先队列 | `FermatPriorityQueue` | 支持 `change`/`remove`；≤2k push 约 2×，1k push/pop 约 4×；实测平台 std pop 在 ~600 附近有断崖 |
| 小对象池化（同线程） | `ObjectPool` | 线程本地缓存，比 `new`/`delete` 快 5~10 倍 |
...
| | 1024 | 301 | **273** | fermat |
| **RandomAccessSmall** | 4 | 174 | **174** | 持平 |
| | 1024 | **172** | 180 | std |
...
|------|------|---------------------|----------------------|------|
| **Push** | 1000 | **1853 ns** | 2369 ns | ~1.3x |
| | 100000 | **980 µs** | 1253 µs | ~1.3x |
| **Pop** | 1000 | **12.6 µs** | 38.7 µs | std 在 ~600 附近有断崖 |
| | 100000 | **7.10 ms** | 7.25 ms | 基本持平 |
| **PushPop** | 1000 | **9.90 µs** | 41.4 µs | ~4.2x |
| | 100000 | **7.82 ms** | 8.19 ms | ~1.05x |
| **ConstructFromIterators** | 10000 | **20.2 µs** | 46.9 µs | ~2.3x |
| **BoundedPushPop**（上限 1000） | – | **13.9 µs** | 33.8 µs | 2000 次流式操作 |
