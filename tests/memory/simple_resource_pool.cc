...
#include <fermat/deprecate/resource_pool.h>
#include <thread>
#include <vector>
#include <atomic>
...
namespace fermat {
    ...
    using TestPool = SimpleResourcePool<TrackedResource, 2, 4, 4, 2>;
    ...
    // -----------------------------------------------------------------------------
    // SimpleResourceId utility tests
    // -----------------------------------------------------------------------------
    TEST(ResourceIdTest, DefaultConstruct) {
        SimpleResourceId rid;
        EXPECT_EQ(rid.encode(), SimpleResourceId::kInvalidId);
    ...
    // -----------------------------------------------------------------------------
    // SimpleResourcePool basic operations
    // -----------------------------------------------------------------------------
    ...
    TEST(ResourcePoolTest, CacheReuse) {
        ...
        SimpleResourceId rid1(id1), rid2(id2);
    ...
    TEST(ResourcePoolTest, CacheLimit) {
        ...
        for (int i = 0; i < 8; ++i) {
            ...
            ASSERT_NE(p, nullptr);
            KLOG(INFO) << SimpleResourceId(id).to_string();
            ids.push_back(id);
        }
        for (int64_t id: ids) {
    ...
    using NonTrivialPool = SimpleResourcePool<NonTrivial, 2, 4, 4, 2>;
