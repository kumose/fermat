...
TEST(BasicAllocatorTest, AllocateDeallocate) {
    ...
    for (size_t i = 0; i < n; ++i) EXPECT_EQ(p[i], static_cast<int>(i));
...
TEST(BasicAllocatorTest, CollectAndApplyArena) {
   ...
   Alloc::apply_arena(guards);
    ...
    { auto cleanup = Alloc::collect_arena(); }   ///< Purge previous state.

    /// Test sizes in terms of number of int elements.
    std::vector<size_t> requests = {
        1,          ///< single element (goes to pool size 1)
        64,         ///< exactly kTinySize
        65,         ///< just above kTinySize -> kSmallSize (256)
        256,        ///< exactly kSmallSize
        257,        ///< just above kSmallSize -> kMediumSize (512)
        512,        ///< exactly kMediumSize
        513,        ///< just above kMediumSize -> kBigSize (1024)
        1024,       ///< exactly kBigSize
        1025,       ///< just above kBigSize -> kPageSize (4096)
        4096,       ///< exactly kPageSize
        4097        ///< above kPageSize -> fallback to generic allocation
    };
    ...
    /// 1. Allocate each size.
    for (size_t req : requests) {
        size_t n = req;
        int* p = alloc.allocate(&n);
        records.push_back({p, req, n});
    }

    /// 2. Deallocate using the actual sizes.
    ...
    /// 4. Verify that each allocation went to the expected pool.
    ///    We map the actual element count (rec.actual) to the expected block size (good_size).
    for (auto& rec : records) {
        ...
        if (actual_elements == 1) {
            ...
            // Should have been rounded up to kTinySize
            ...
            // Generic allocation: actual_elements may be whatever good_alloc returns.
            // No pool involved, so we just check that it's >= requested.
            EXPECT_GE(actual_elements, rec.requested);
            continue;   ///< Not tracked in guards (since generic allocations are not pooled).
        }

        // Check that there is exactly one guard with this block size containing this pointer.
        bool found = false;
        for (auto& g : guards) {
    ...
    {
        auto cleanup = Alloc::collect_arena();
        // All blocks should be collected; their destructor will free them.
    }
