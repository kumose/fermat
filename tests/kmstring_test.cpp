...
#include <fermat/conv.h>
#include <fermat/portability.h>
#include <fermat/random.h>
#include <fermat/utility.h>
#include <fermat/container/foreach.h>
#include <fermat/portability/gtest.h>
#include <fermat/test/test_utils.h>

using namespace std;
using namespace melon;

template <typename A, typename B>
using detect_eq = decltype(MELON_DECLVAL(A) == MELON_DECLVAL(B));
template <typename A, typename B>
using detect_ne = decltype(MELON_DECLVAL(A) != MELON_DECLVAL(B));
template <typename A, typename B>
using detect_lt = decltype(MELON_DECLVAL(A) < MELON_DECLVAL(B));
template <typename A, typename B>
using detect_le = decltype(MELON_DECLVAL(A) <= MELON_DECLVAL(B));
template <typename A, typename B>
using detect_gt = decltype(MELON_DECLVAL(A) > MELON_DECLVAL(B));
template <typename A, typename B>
using detect_ge = decltype(MELON_DECLVAL(A) >= MELON_DECLVAL(B));
#if MELON_CPLUSPLUS >= 202002
template <typename A, typename B>
using detect_3w = decltype(MELON_DECLVAL(A) <=> MELON_DECLVAL(B));
#endif

static_assert(!std::is_constructible_v<kmstring, nullptr_t>);
static_assert(!std::is_assignable_v<kmstring, nullptr_t>);

static_assert(!is_detected_v<detect_eq, kmstring, nullptr_t>);
static_assert(!is_detected_v<detect_ne, kmstring, nullptr_t>);
static_assert(!is_detected_v<detect_lt, kmstring, nullptr_t>);
static_assert(!is_detected_v<detect_le, kmstring, nullptr_t>);
static_assert(!is_detected_v<detect_gt, kmstring, nullptr_t>);
static_assert(!is_detected_v<detect_ge, kmstring, nullptr_t>);

static_assert(!is_detected_v<detect_eq, nullptr_t, kmstring>);
static_assert(!is_detected_v<detect_ne, nullptr_t, kmstring>);
static_assert(!is_detected_v<detect_lt, nullptr_t, kmstring>);
static_assert(!is_detected_v<detect_le, nullptr_t, kmstring>);
static_assert(!is_detected_v<detect_gt, nullptr_t, kmstring>);
static_assert(!is_detected_v<detect_ge, nullptr_t, kmstring>);

#if MELON_CPLUSPLUS >= 202002
static_assert(!is_detected_v<detect_3w, kmstring, nullptr_t>);
static_assert(!is_detected_v<detect_3w, nullptr_t, kmstring>);
#endif

namespace {

static const int seed = melon::randomNumberSeed();
...
void Num2String(String& str, Integral n) {
  std::string tmp = melon::to<std::string>(n);
...
void clause11_21_4_2_k(String& test) {
  ...
  test = std::move(s);
  if (std::is_same<String, kmstring>::value) {
...
TEST(KMString, testAllClauses) {
  EXPECT_TRUE(1) << "Starting with seed: " << seed;
  std::string r;
  melon::kmstring c;
#if MELON_HAVE_WCHAR_SUPPORT
  std::wstring wr;
  melon::basic_kmstring<wchar_t> wc;
#endif
  int count = 0;

  auto l = [&](const char* const clause,
               void (*f_string)(std::string&),
               void (*f_kmstring)(melon::kmstring&),
               void (*f_wkmstring)(melon::basic_kmstring<wchar_t>&)) {
    do {
      if (true) {
      } else {
      ...
      wc = melon::basic_kmstring<wchar_t>(wr.c_str());
#endif
...
#define TEST_CLAUSE(x)             \
  l(#x,                            \
    ...
    clause11_##x<melon::kmstring>, \
    clause11_##x<melon::basic_kmstring<wchar_t>>);
...
sed nisl. In diam lacus, lobortis ut posuere nec, ornare id quam.";
  ...
  vector<kmstring> v = turbo::str_split(s1,turbo::ByAnyChar("\n"));
  {
...
TEST(KMString, testMoveCtor) {
  ...
  kmstring s(size, 'a');
  kmstring test = std::move(s);
  EXPECT_TRUE(s.empty());
  EXPECT_EQ(size, test.size());
}

TEST(KMString, testMoveAssign) {
  // Move constructor. Make sure we allocate a large string, so the
  // small string optimization doesn't kick in.
  auto size = random(100, 2000);
  kmstring s(size, 'a');
...
TEST(KMString, testMoveOperatorPlusLhs) {
...
TEST(KMString, testMoveOperatorPlusRhs) {
  // Make sure we allocate a large string, so the
  // small string optimization doesn't kick in.
  auto size1 = random(100, 2000);
  auto size2 = random(100, 2000);
...
TEST(KMString, testConstructionFromLiteralZero) {
  EXPECT_THROW(kmstring s(static_cast<const char*>(nullptr)), std::logic_error);
}

TEST(KMString, testFixedBugsD479397) {
  kmstring str(1337, 'f');
  kmstring cp = str;
  cp.clear();
  cp.c_str();
...
TEST(KMString, testFixedBugsD481173) {
  kmstring str(1337, 'f');
  for (int i = 0; i < 2; ++i) {
    kmstring cp = str;
    cp[1] = 'b';
...
TEST(KMString, testFixedBugsD580267PushBack) {
  kmstring str(1337, 'f');
  kmstring cp = str;
  cp.push_back('f');
}

TEST(KMString, testFixedBugsD580267OperatorAddAssign) {
  kmstring str(1337, 'f');
  kmstring cp = str;
  cp += "bb";
}

TEST(KMString, testFixedBugsD661622) {
  melon::basic_kmstring<wchar_t> s;
  EXPECT_EQ(0, s.size());
}

TEST(KMString, testFixedBugsD785057) {
  kmstring str(1337, 'f');
  std::swap(str, str);
  EXPECT_EQ(1337, str.size());
}

TEST(KMString, testFixedBugsD1012196AllocatorMalloc) {
  kmstring str(128, 'f');
  str.clear(); // Empty medium string.
  kmstring copy(str); // Medium string of 0 capacity.
  copy.push_back('b');
  EXPECT_GE(copy.capacity(), 1);
}

TEST(KMString, testFixedBugsD2813713) {
  kmstring s1("a");
...
TEST(KMString, testFixedBugsD3698862) {
  EXPECT_EQ(kmstring().find(kmstring(), 4), kmstring::npos);
}

TEST(KMString, testFixedBugsD4355440) {
...
TEST(KMString, findWithNpos) {
  kmstring fbstr("localhost:80");
  EXPECT_EQ(kmstring::npos, fbstr.find(":", kmstring::npos));
}

TEST(KMString, testHash) {
  kmstring a;
  kmstring b;
  a.push_back(0);
  a.push_back(1);
  b.push_back(0);
  b.push_back(2);
  std::hash<kmstring> hashfunc;
  EXPECT_NE(hashfunc(a), hashfunc(b));
}

#if MELON_HAVE_WCHAR_SUPPORT
TEST(KMString, testHashChar16) {
  u16kmstring a;
  u16kmstring b;
  a.push_back(0);
  a.push_back(1);
  b.push_back(0);
  b.push_back(2);
  std::hash<u16kmstring> hashfunc;
  EXPECT_NE(hashfunc(a), hashfunc(b));
}
#endif

TEST(KMString, testFrontBack) {
  kmstring str("hello");
...
TEST(KMString, noexcept) {
  EXPECT_TRUE(noexcept(kmstring()));
  kmstring x;
  EXPECT_FALSE(noexcept(kmstring(x)));
  EXPECT_TRUE(noexcept(kmstring(std::move(x))));
  kmstring y;
  EXPECT_FALSE(noexcept(y = x));
  EXPECT_TRUE(noexcept(y = std::move(x)));
}

TEST(KMString, iomanip) {
  stringstream ss;
  kmstring fbstr("Hello");
...
TEST(KMString, moveTerminator) {
  // The source of a move must remain in a valid state
  kmstring s(100, 'x'); // too big to be in-situ
  kmstring k;
  k = std::move(s);
 ...
 *   explicit basic_kmstring(const A& a) noexcept;
 */

struct TestStructDefaultAllocator {
  melon::basic_kmstring<char> stringMember;
};
...
TEST(KMStringCtorTest, DefaultInitStructDefaultAlloc) {
  TestStructDefaultAllocator t1{};
  EXPECT_TRUE(t1.stringMember.empty());
}

TEST(KMStringCtorTest, NullZeroConstruction) {
  char* p = nullptr;
  int n = 0;
  melon::kmstring f(p, n);
  EXPECT_EQ(f.size(), 0);
}
...
TEST(KMString, compareToStdString) {
  using melon::kmstring;
...
TEST(U16KMString, compareToStdU16String) {
  using melon::basic_kmstring;
  using namespace std::string_literals;
  auto stdA = u"a"s;
  auto stdB = u"b"s;
  basic_kmstring<char16_t> fbA(u"a");
  basic_kmstring<char16_t> fbB(u"b");
  EXPECT_TRUE(stdA == fbA);
  EXPECT_TRUE(fbB == stdB);
  EXPECT_TRUE(stdA != fbB);
  EXPECT_TRUE(fbA != stdB);
...
TEST(U32KMString, compareToStdU32String) {
  using melon::basic_kmstring;
  using namespace std::string_literals;
  auto stdA = U"a"s;
  auto stdB = U"b"s;
  basic_kmstring<char32_t> fbA(U"a");
  basic_kmstring<char32_t> fbB(U"b");
  EXPECT_TRUE(stdA == fbA);
  EXPECT_TRUE(fbB == stdB);
  EXPECT_TRUE(stdA != fbB);
  EXPECT_TRUE(fbA != stdB);
...
TEST(WKMString, compareToStdWString) {
  using melon::basic_kmstring;
  using namespace std::string_literals;
  auto stdA = L"a"s;
  auto stdB = L"b"s;
  basic_kmstring<wchar_t> fbA(L"a");
  basic_kmstring<wchar_t> fbB(L"b");
  EXPECT_TRUE(stdA == fbA);
  EXPECT_TRUE(fbB == stdB);
  EXPECT_TRUE(stdA != fbB);
  EXPECT_TRUE(fbA != stdB);
...
TEST(KMString, compareToStdStringLong) {
  using melon::kmstring;
  using namespace std::string_literals;
  auto stdA = "1234567890a"s;
  auto stdB = "1234567890ab"s;
  kmstring fbA("1234567890a");
  kmstring fbB("1234567890ab");
  EXPECT_TRUE(stdA == fbA);
  EXPECT_TRUE(fbB == stdB);
  EXPECT_TRUE(stdA != fbB);
  EXPECT_TRUE(fbA != stdB);
...
TEST(U16KMString, compareToStdU16StringLong) {
  using melon::basic_kmstring;
  using namespace std::string_literals;
  auto stdA = u"1234567890a"s;
  auto stdB = u"1234567890ab"s;
  basic_kmstring<char16_t> fbA(u"1234567890a");
  basic_kmstring<char16_t> fbB(u"1234567890ab");
  EXPECT_TRUE(stdA == fbA);
  EXPECT_TRUE(fbB == stdB);
  EXPECT_TRUE(stdA != fbB);
  EXPECT_TRUE(fbA != stdB);
...
#if MELON_HAVE_WCHAR_SUPPORT
TEST(U32KMString, compareToStdU32StringLong) {
  using melon::basic_kmstring;
  using namespace std::string_literals;
  auto stdA = U"1234567890a"s;
  auto stdB = U"1234567890ab"s;
  basic_kmstring<char32_t> fbA(U"1234567890a");
  basic_kmstring<char32_t> fbB(U"1234567890ab");
  EXPECT_TRUE(stdA == fbA);
  EXPECT_TRUE(fbB == stdB);
  EXPECT_TRUE(stdA != fbB);
  EXPECT_TRUE(fbA != stdB);
...
TEST(WKMString, compareToStdWStringLong) {
  using melon::basic_kmstring;
  using namespace std::string_literals;
  auto stdA = L"1234567890a"s;
  auto stdB = L"1234567890ab"s;
  basic_kmstring<wchar_t> fbA(L"1234567890a");
  basic_kmstring<wchar_t> fbB(L"1234567890ab");
  EXPECT_TRUE(stdA == fbA);
  EXPECT_TRUE(fbB == stdB);
  EXPECT_TRUE(stdA != fbB);
  EXPECT_TRUE(fbA != stdB);
...
TEST(KMString, convertFromStringView) {
  {
    melon::kmstring test{std::string_view("foo")};
    std::string control{std::string_view("foo")};
    EXPECT_EQ(test, "foo");
    EXPECT_EQ(test, control);
  }
  {
    melon::kmstring test{std::string_view("abcfooabc"), 3, 3};
  ...
  }
  {
    ...
    melon::basic_kmstring<char, custom_traits> test{sv_type("foo")};
  ...
  {
    using sv_type = std::basic_string_view<char, custom_traits>;
    melon::basic_kmstring<char, custom_traits> test{sv_type("abcfooabc"), 3, 3};
...
TEST(KMString, convertToStringView) {
  melon::kmstring s("foo");
  std::string_view sv = s;
  EXPECT_EQ(sv, "foo");
  melon::basic_kmstring<char, custom_traits> s2("bar");
  std::basic_string_view<char, custom_traits> sv2 = s2;
  EXPECT_EQ(sv2, "bar");
}

TEST(KMString, Format) {
  EXPECT_EQ("  foo", fmt::format("{:>5}", melon::kmstring("foo")));
}

TEST(KMString, OverLarge) {
  EXPECT_THROW(
      kmstring().reserve((size_t)0xFFFF'FFFF'FFFF'FFFF), std::length_error);
  EXPECT_THROW(
      kmstring_core<char32_t>().reserve((size_t)0x4000'0000'4000'0000),
      std::length_error);
}

#if MELON_CPLUSPLUS >= 202002L

TEST(KMString, SpaceshipOperator) {
  melon::kmstring a{"a"};
