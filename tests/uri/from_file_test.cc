...
std::string long_way(std::string path) {
  fermat::uri::result<fermat::uri::Url> base = fermat::uri::parse<fermat::uri::Url>("file://");
  base->set_pathname(path);
  return std::string(base->get_href());
}
...
TEST(from_file_tests, basics) {
  for (std::string path :
    ...
    ASSERT_TRUE(long_way(path) == fermat::uri::href_from_file(path));
  }
}
