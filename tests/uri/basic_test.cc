...
TYPED_TEST(basic_tests, insane_url) {
  auto r = fermat::uri::parse<fermat::uri::UrlAggregator>("e:@EEEEEEEEEE");
  ASSERT_TRUE(r);
...
TYPED_TEST(basic_tests, bad_percent_encoding) {
  auto r = fermat::uri::parse<TypeParam>("http://www.google.com/%X%");
  ASSERT_TRUE(r);
  ASSERT_EQ(r->get_href(), "http://www.google.com/%X%");
  r = fermat::uri::parse<TypeParam>("http://www.google%X%.com/");
  ASSERT_FALSE(r);
  r = fermat::uri::parse<TypeParam>("http://www.google.com/");
  ASSERT_TRUE(r);
...
TYPED_TEST(basic_tests, spaces_spaces) {
  auto r = fermat::uri::parse<TypeParam>("http://www.google.com/%37/ /");
  ASSERT_TRUE(r);
...
TYPED_TEST(basic_tests, set_host_should_return_false_sometimes) {
  auto r = fermat::uri::parse<TypeParam>("mailto:a@b.com");
  ASSERT_FALSE(r->set_host("something"));
  auto r2 = fermat::uri::parse<TypeParam>("mailto:a@b.com");
  ASSERT_FALSE(r2->set_host("something"));
  SUCCEED();
}

TYPED_TEST(basic_tests, empty_url_should_return_false) {
  auto r = fermat::uri::parse<TypeParam>("");
  ASSERT_FALSE(r);
  SUCCEED();
}

TYPED_TEST(basic_tests, set_host_should_return_true_sometimes) {
  auto r = fermat::uri::parse<TypeParam>("https://www.google.com");
...
TYPED_TEST(basic_tests, readme) {
  auto url = fermat::uri::parse<TypeParam>("https://www.google.com");
...
TYPED_TEST(basic_tests, set_protocol_should_return_false_sometimes) {
  auto url = fermat::uri::parse<TypeParam>("file:");
...
TYPED_TEST(basic_tests, nodejs1) {
  auto base = fermat::uri::parse<TypeParam>("http://other.com/");
  ASSERT_TRUE(base.has_value());
  auto url = fermat::uri::parse<TypeParam>("http://GOOgoo.com", &base.value());
  ASSERT_TRUE(url.has_value());
  SUCCEED();
}

TYPED_TEST(basic_tests, nodejs2) {
  auto url = fermat::uri::parse<TypeParam>("data:space    ?test");
...
TYPED_TEST(basic_tests, nodejs3) {
  auto url = fermat::uri::parse<TypeParam>("data:space    ?test#test");
  ASSERT_EQ(url->get_search(), "?test");
  url->set_search("");
  ASSERT_EQ(url->get_search(), "");
  ASSERT_EQ(url->get_pathname(), "space    ");
...
TYPED_TEST(basic_tests, nodejs4) {
  auto url = fermat::uri::parse<TypeParam>("file:///var/log/system.log");
...
TYPED_TEST(basic_tests, empty_url) {
  auto url = fermat::uri::parse<TypeParam>("");
  ASSERT_FALSE(url);
  SUCCEED();
}

TYPED_TEST(basic_tests, just_hash) {
  auto url = fermat::uri::parse<TypeParam>("#x");
  ASSERT_FALSE(url);
  SUCCEED();
}

TYPED_TEST(basic_tests, empty_host_dash_dash_path) {
  auto url = fermat::uri::parse<TypeParam>("something:/.//");
  ASSERT_TRUE(url);
...
TYPED_TEST(basic_tests, confusing_mess) {
  auto base_url = fermat::uri::parse<TypeParam>("http://example.org/foo/bar");
  ASSERT_TRUE(base_url);
  auto url = fermat::uri::parse<TypeParam>("http://::@c@d:2", &*base_url);
  ASSERT_TRUE(url);
  ASSERT_FALSE(url->has_opaque_path);
...
TYPED_TEST(basic_tests, standard_file) {
  auto url = fermat::uri::parse<TypeParam>("file:///tmp/mock/path");
  ASSERT_TRUE(url);
...
TYPED_TEST(basic_tests, default_port_should_be_removed) {
  auto url = fermat::uri::parse<TypeParam>("http://www.google.com:443");
  ASSERT_TRUE(url);
...
TYPED_TEST(basic_tests, test_amazon) {
  auto url = fermat::uri::parse<TypeParam>("HTTP://AMAZON.COM");
  ASSERT_TRUE(url);
...
TYPED_TEST(basic_tests, remove_username) {
  auto url = fermat::uri::parse<TypeParam>("http://me@example.net");
  ASSERT_TRUE(url);
  url->set_username("");
...
TYPED_TEST(basic_tests, remove_password) {
  auto url = fermat::uri::parse<TypeParam>("http://user:pass@example.net");
  ASSERT_TRUE(url);
  url->set_password("");
...
TYPED_TEST(basic_tests, remove_password_with_empty_username) {
  auto url = fermat::uri::parse<TypeParam>("http://:pass@example.net");
  ASSERT_TRUE(url);
  url->set_password("");
  ASSERT_EQ(url->get_username(), "");
  ASSERT_EQ(url->get_password(), "");
...
TYPED_TEST(basic_tests, should_remove_dash_dot) {
  auto url = fermat::uri::parse<TypeParam>("non-spec:/.//p");
  ASSERT_TRUE(url);
...
TYPED_TEST(basic_tests, should_add_dash_dot_on_pathname) {
  auto url = fermat::uri::parse<TypeParam>("non-spec:/");
  ASSERT_TRUE(url);
  url->set_pathname("//p");
  ASSERT_EQ(url->get_pathname(), "//p");
  ASSERT_EQ(url->get_href(), "non-spec:/.//p");
...
TYPED_TEST(basic_tests, node_issue_47889) {
  auto urlbase = fermat::uri::parse<TypeParam>("a:b");
  ...
  ASSERT_TRUE(urlbase);
  auto expected_url = fermat::uri::parse<TypeParam>("a:b#");
  ...
  auto url = fermat::uri::parse<TypeParam>("..#", &*urlbase);
  ASSERT_TRUE(url);
...
TEST(basic_tests, can_parse) {
  ASSERT_TRUE(fermat::uri::can_parse("https://www.yagiz.co"));
  std::string_view base = "https://yagiz.co";
  ASSERT_TRUE(fermat::uri::can_parse("/hello", &base));

  std::string_view invalid_base = "!!!!!!!1";
  ASSERT_FALSE(fermat::uri::can_parse("/hello", &invalid_base));
  ASSERT_FALSE(fermat::uri::can_parse("!!!"));
  SUCCEED();
}

TYPED_TEST(basic_tests, node_issue_48254) {
  auto base_url = fermat::uri::parse<TypeParam>("localhost:80");
  ASSERT_TRUE(base_url);
  ...
  auto url = fermat::uri::parse<TypeParam>("", &*base_url);
  ASSERT_FALSE(url);
  SUCCEED();
}

TYPED_TEST(basic_tests, UrlHostType) {
  ASSERT_EQ(fermat::uri::parse<TypeParam>("http://localhost:3000")->host_type,
            fermat::uri::UrlHostType::DEFAULT);
  ASSERT_EQ(fermat::uri::parse<TypeParam>("http://0.0.0.0")->host_type,
            fermat::uri::UrlHostType::IPV4);
  ASSERT_EQ(
      fermat::uri::parse<TypeParam>("http://[2001:db8:3333:4444:5555:6666:7777:8888]")
          ->host_type,
      fermat::uri::UrlHostType::IPV6);
  SUCCEED();
}

// https://github.com/nodejs/node/issues/49650
TYPED_TEST(basic_tests, nodejs_49650) {
  auto out = fermat::uri::parse<TypeParam>("http://foo");
  ASSERT_TRUE(out);
...
TYPED_TEST(basic_tests, nodejs_50235) {
  auto out = fermat::uri::parse<TypeParam>("http://test.com:5/?param=1");
  ASSERT_TRUE(out);
...
TYPED_TEST(basic_tests, nodejs_51514) {
  auto out = fermat::uri::parse<TypeParam>("http://1.1.1.256");
  ASSERT_FALSE(out);
}

// https://github.com/nodejs/node/issues/51593
TYPED_TEST(basic_tests, nodejs_51593) {
  auto out = fermat::uri::parse<TypeParam>("http://\u200b123.123.123.123");
  ASSERT_TRUE(out);
...
TYPED_TEST(basic_tests, nodejs_51619) {
  auto out = fermat::uri::parse<TypeParam>("https://0.0.0.0x100/");
  ASSERT_FALSE(out);
  SUCCEED();
}
...
TYPED_TEST(basic_tests, nodejs_undici_2971) {
  ...
  auto base_url = fermat::uri::parse<TypeParam>(base);
  ASSERT_TRUE(base_url);
  auto out = fermat::uri::parse<TypeParam>("/\xec\x95\x88\xeb\x85\x95", &*base_url);
  ASSERT_TRUE(out);
  ASSERT_EQ(
      out->get_href(),
...
TYPED_TEST(basic_tests, path_setter_bug) {
  ...
  auto base_url = fermat::uri::parse<fermat::uri::UrlAggregator>(base);
  ASSERT_TRUE(base_url);
