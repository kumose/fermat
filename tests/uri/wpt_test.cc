...
std::string gen_file_path(std::string_view view) {
   return  turbo::str_format("%s/uri/wpt/%s", testing::testing_root(), view);
}

std::string PERCENT_ENCODING_JSON = gen_file_path( "percent-encoding.json");
std::string SETTERS_TESTS_JSON = gen_file_path( "setters_tests.json");
std::string ADA_SETTERS_TESTS_JSON =gen_file_path( "ada_extra_setters_tests.json");
std::string TOASCII_JSON = gen_file_path("toascii.json");
std::string IDNA_TEST_V2 = gen_file_path( "IdnaTestV2.json");
std::string URLTESTDATA_JSON = gen_file_path( "urltestdata.json");
std::string ADA_URLTESTDATA_JSON = gen_file_path( "ada_extra_urltestdata.json");
...
TEST(wpt_tests, verify_dns_length) {
    auto source = VERIFYDNSLENGTH_TESTS_JSON;
    size_t counter{};
    ondemand::parser parser;
    ASSERT_TRUE(file_exists(source));
    padded_string json = padded_string::load(source);
