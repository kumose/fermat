...
void test_search_n_iter() {
    ...
    auto res = fermat::ranges::search_n(a, a + N, 0, 0);
    EXPECT_EQ(res.begin(), a);
    EXPECT_EQ(res.end(), a);

    // count = 1, value = 0
    res = fermat::ranges::search_n(a, a + N, 1, 0);
    EXPECT_EQ(res.begin(), a);
    EXPECT_EQ(res.end(), a + 1);

    // count = 2, value = 0 (not present)
    res = fermat::ranges::search_n(a, a + N, 2, 0);
    EXPECT_EQ(res.begin(), a + N);
    EXPECT_EQ(res.end(), a + N);

    // count = N, value = 0 (not enough)
    res = fermat::ranges::search_n(a, a + N, N, 0);
    EXPECT_EQ(res.begin(), a + N);
    EXPECT_EQ(res.end(), a + N);

    // count = 0, value = 3
    res = fermat::ranges::search_n(a, a + N, 0, 3);
    EXPECT_EQ(res.begin(), a);
    EXPECT_EQ(res.end(), a);

    // count = 1, value = 3
    res = fermat::ranges::search_n(a, a + N, 1, 3);
    EXPECT_EQ(res.begin(), a + 3);
    EXPECT_EQ(res.end(), a + 4);

    // count = 2, value = 3 (only one)
    res = fermat::ranges::search_n(a, a + N, 2, 3);
    EXPECT_EQ(res.begin(), a + N);
    EXPECT_EQ(res.end(), a + N);

    // count = 1, value = 5 (last element)
    res = fermat::ranges::search_n(a, a + N, 1, 5);
    EXPECT_EQ(res.begin(), a + 5);
    EXPECT_EQ(res.end(), a + 6);

    // count = 2, value = 5 (only one)
    res = fermat::ranges::search_n(a, a + N, 2, 5);
    EXPECT_EQ(res.begin(), a + N);
    EXPECT_EQ(res.end(), a + N);
    ...
    res = fermat::ranges::search_n(c, c + L, 1, 0);
    EXPECT_EQ(res.begin(), c);
    EXPECT_EQ(res.end(), c + 1);

    res = fermat::ranges::search_n(c, c + L, 2, 0);
    EXPECT_EQ(res.begin(), c);
    EXPECT_EQ(res.end(), c + 2);

    res = fermat::ranges::search_n(c, c + L, 3, 0);
    EXPECT_EQ(res.begin(), c);
    EXPECT_EQ(res.end(), c + 3);

    res = fermat::ranges::search_n(c, c + L, 4, 0);
...
void test_search_n_range() {
    int a[] = {0, 1, 2, 3, 4, 5};
    const std::size_t N = sizeof(a)/sizeof(a[0]);
    auto rng = fermat::ranges::make_subrange(a, a + N);

    auto res = fermat::ranges::search_n(rng, 1, 0);
    EXPECT_EQ(res.begin(), a);
    EXPECT_EQ(res.end(), a + 1);

    res = fermat::ranges::search_n(rng, 2, 0);
    EXPECT_EQ(res.begin(), a + N);
    EXPECT_EQ(res.end(), a + N);

    res = fermat::ranges::search_n(rng, 1, 3);
    EXPECT_EQ(res.begin(), a + 3);
    EXPECT_EQ(res.end(), a + 4);

    int b[] = {0, 0, 1, 1, 2, 2};
    auto rng2 = fermat::ranges::make_subrange(b, b + 6);
    res = fermat::ranges::search_n(rng2, 2, 1);
    EXPECT_EQ(res.begin(), b + 2);
    EXPECT_EQ(res.end(), b + 4);
}
...
TEST(SearchNTest, Projection) {
    S arr[] = {{0}, {1}, {2}, {2}, {4}, {5}};
    auto res = fermat::ranges::search_n(arr, 2, 2, std::equal_to<int>{}, &S::i);
...
TEST(SearchNTest, CountedRange) {
    ...
    // Use fermat::ranges::distance to get the length of the subrange
...
TEST(SearchNTest, RvalueRange) {
    int ib[] = {0, 0, 1, 1, 2, 2};
    // lvalue view
    auto res = fermat::ranges::search_n(fermat::ranges::views::all(ib), 2, 1);
    EXPECT_EQ(res.begin(), ib + 2);
    EXPECT_EQ(res.end(), ib + 4);

    // rvalue array (moved)
    int ib2[] = {0, 0, 1, 1, 2, 2};
    auto res2 = fermat::ranges::search_n(std::move(ib2), 2, 1);
    // res2 is dangling; we only check that the algorithm runs (no runtime check)
    (void)res2;

    // rvalue vector
    std::vector<int> vec{0, 0, 1, 1, 2, 2};
    auto res3 = fermat::ranges::search_n(std::move(vec), 2, 1);
    (void)res3;
}
...
TEST(SearchNTest, Constexpr) {
    constexpr auto test = []() constexpr {
        int arr[] = {0, 1, 2, 2, 4, 5};
        auto res = fermat::ranges::search_n(arr, 2, 2, std::equal_to<int>{});
