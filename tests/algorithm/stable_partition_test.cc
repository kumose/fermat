...
void test_range() {
    ...
    {  // mixed
        P ap[] = { {0, 1}, {0, 2}, {1, 1}, {1, 2}, {2, 1}, {2, 2}, {3, 1}, {3, 2}, {4, 1}, {4, 2} };
        std::size_t size = ranges::size(ap);
        Iter r = ranges::stable_partition(as_lvalue(make_subrange(Iter(ap), Sent(ap+size))), odd_first());
        EXPECT_EQ(base(r), ap + 4);
        EXPECT_EQ(ap[0], (P{1, 1}));
        EXPECT_EQ(ap[1], (P{1, 2}));
        EXPECT_EQ(ap[2], (P{3, 1}));
    ...
    }
    {
        ...
        EXPECT_EQ(ap[9], (P{4, 2}));
        // empty
        r = ranges::stable_partition(as_lvalue(make_subrange(Iter(ap), Sent(ap))), odd_first());
        EXPECT_EQ(base(r), ap);
        // one true
        r = ranges::stable_partition(as_lvalue(make_subrange(Iter(ap), Sent(ap+1))), odd_first());
        EXPECT_EQ(base(r), ap + 1);
        EXPECT_EQ(ap[0], (P{1, 1}));
        // one false
        r = ranges::stable_partition(as_lvalue(make_subrange(Iter(ap+4), Sent(ap+5))), odd_first());
        EXPECT_EQ(base(r), ap + 4);
        EXPECT_EQ(ap[4], (P{0, 1}));
    }
    {  // all false
