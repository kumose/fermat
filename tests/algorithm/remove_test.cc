...
void test_remove_iter() {
    ...
    int* r = fermat::ranges::remove(ia, ia + sa, 2);
    EXPECT_EQ(r, ia + sa - 3);
    EXPECT_EQ(ia[0], 0);
    EXPECT_EQ(ia[1], 1);
    EXPECT_EQ(ia[2], 3);
...
void test_remove_range() {
    ...
    auto r = fermat::ranges::remove(fermat::ranges::make_subrange(ia, ia + sa), 2);
    EXPECT_EQ(r, ia + sa - 3);
    EXPECT_EQ(ia[0], 0);
    EXPECT_EQ(ia[1], 1);
    EXPECT_EQ(ia[2], 3);
...
void test_remove_move_only() {
    ...
    auto r = fermat::ranges::remove(ia, ia + sa, std::unique_ptr<int>());
    EXPECT_EQ(r, ia + sa - 3);
    EXPECT_EQ(*ia[0], 0);
    EXPECT_EQ(*ia[1], 1);
    EXPECT_EQ(*ia[2], 3);
...
TEST(RemoveTest, Projection) {
    ...
    S* r = fermat::ranges::remove(arr, 2, &S::i);
...
TEST(RemoveTest, RvalueRange) {
    S arr[] = {S{0}, S{1}, S{2}, S{3}, S{4}, S{2}, S{3}, S{4}, S{2}};
    auto r = fermat::ranges::remove(std::move(arr), 2, &S::i);
    ...
    auto r2 = fermat::ranges::remove(std::move(vec), 2, &S::i);
    (void)r2;
    EXPECT_EQ(vec[0].i, 0);
    EXPECT_EQ(vec[1].i, 1);
    EXPECT_EQ(vec[2].i, 3);
...
TEST(RemoveTest, Constexpr) {
    constexpr auto test = []() constexpr {
        ...
        constexpr std::size_t sa = sizeof(arr) / sizeof(arr[0]);
        int* r = fermat::ranges::remove(arr, 2);
        bool ok = (r == arr + sa - 3) &&
                  (arr[0] == 0) && (arr[1] == 1) &&
                  (arr[2] == 3) && (arr[3] == 4) &&
                  (arr[4] == 3) && (arr[5] == 4);
