...
void not_totally_ordered() {
    ...
    fermat::ranges::equal_range(vec, my_int{10}, compare);
}

template<class Iter, class Sent, class T, class Proj = fermat::ranges::identity>
void test_it(Iter first, Sent last, const T& value, Proj proj = Proj{}) {
    auto i = fermat::ranges::equal_range(first, last, value, fermat::ranges::less{}, proj);
    for (Iter j = first; j != i.begin(); ++j)
        EXPECT_LT(fermat::ranges::invoke(proj, *j), value);
    for (Iter j = i.begin(); j != last; ++j)
        EXPECT_FALSE(fermat::ranges::invoke(proj, *j) < value);
    for (Iter j = first; j != i.end(); ++j)
        EXPECT_FALSE(value < fermat::ranges::invoke(proj, *j));
    for (Iter j = i.end(); j != last; ++j)
        EXPECT_LT(value, fermat::ranges::invoke(proj, *j));

    auto res = fermat::ranges::equal_range(fermat::ranges::make_subrange(first, last), value, fermat::ranges::less{}, proj);
...
TEST(EqualRangeTest, Constexpr) {
    constexpr auto test = []() constexpr {
        ...
        auto res = fermat::ranges::equal_range(d, 2);
