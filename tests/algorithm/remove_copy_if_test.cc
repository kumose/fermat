...
void test_remove_copy_if_iter() {
    ...
    auto res = fermat::ranges::remove_copy_if(ia, ia + sa, ib, is_two);
...
void test_remove_copy_if_range() {
    ...
    auto res = fermat::ranges::remove_copy_if(rng, ib, is_two);
    EXPECT_EQ(res.in, ia + sa);
    EXPECT_EQ(res.out, ib + (sa - 3));
    EXPECT_EQ(ib[0], 0);
    EXPECT_EQ(ib[1], 1);
...
TEST(RemoveCopyIfTest, Projection) {
    ...
    auto res = fermat::ranges::remove_copy_if(ia, ib, pred, &S::i);
    EXPECT_EQ(res.in, ia + sa);
    EXPECT_EQ(res.out, ib + (sa - 3));
    EXPECT_EQ(ib[0].i, 0);
    EXPECT_EQ(ib[1].i, 1);
...
TEST(RemoveCopyIfTest, RvalueRange) {
    ...
    auto pred = [](int i) { return i == 2; };
    auto res = fermat::ranges::remove_copy_if(std::move(ia), ib, pred, &S::i);
    ...
    auto res2 = fermat::ranges::remove_copy_if(std::move(vec), ib, pred, &S::i);
...
TEST(RemoveCopyIfTest, Constexpr) {
    constexpr auto test = []() constexpr {
        ...
        int ib[6] = {0};
        auto pred = [](int i) { return i == 2; };
        auto r = fermat::ranges::remove_copy_if(ia, ib, pred);
        bool ok = (r.in == ia + sa) &&
                  (r.out == ib + (sa - 3)) &&
                  (ib[0] == 0) && (ib[1] == 1) &&
                  (ib[2] == 3) && (ib[3] == 4) &&
