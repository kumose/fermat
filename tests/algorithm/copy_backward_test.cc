...
constexpr bool test_constexpr() {
    ...
    auto res = fermat::ranges::copy_backward(input, fermat::ranges::end(a1));
    bool ok = (res.in == fermat::ranges::end(input)) &&
              (res.out == fermat::ranges::begin(a1)) &&
              fermat::ranges::equal(a1, IL{0, 1, 2, 3});
    return ok;
}
...
TEST(CopyBackwardTest, PairArray) {
    ...
    {
        auto res = fermat::ranges::copy_backward(begin(a), end(a), end(out));
...

    {
        ...
        auto res = fermat::ranges::copy_backward(a, end(out));
        EXPECT_EQ(res.in, end(a));
        EXPECT_EQ(res.out, begin(out));
        EXPECT_TRUE(std::equal(a, a + size(a), out));
    }
    ...
    {
        std::fill_n(out, size(out), std::make_pair(0, 0));
        auto res = fermat::ranges::copy_backward(std::move(a), end(out));
...

    {
        ...
        auto res = fermat::ranges::copy_backward(std::move(vec), end(out));
        EXPECT_EQ(res.out, begin(out));
        EXPECT_TRUE(std::equal(a, a + size(a), out));
    }
    ...
    {
        std::fill_n(out, size(out), std::make_pair(0, 0));
        auto res = fermat::ranges::copy_backward(fermat::ranges::views::all(a), end(out));
        EXPECT_EQ(res.in, end(a));
        EXPECT_EQ(res.out, begin(out));
        EXPECT_TRUE(std::equal(a, a + size(a), out));
    }
