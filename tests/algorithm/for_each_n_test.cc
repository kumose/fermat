...
#include <fermat/algorithm/for_each.h>   ///< for fermat::ranges::ref
...
TEST(ForEachNTest, Basic) {
    ...
    auto it = fermat::ranges::for_each_n(v1.begin(), 2, fun);
    EXPECT_EQ(it, v1.begin() + 2);
    EXPECT_EQ(sum, 3); // 1+2=3

    sum = 0;
    it = fermat::ranges::for_each_n(v1, 2, fun);
    EXPECT_EQ(it, v1.begin() + 2);
    EXPECT_EQ(sum, 3);
}
...
TEST(ForEachNTest, ReferenceLambda) {
    ...
    auto it = fermat::ranges::for_each_n(v1.begin(), sz, rfun);
    EXPECT_EQ(it, v1.end());
    EXPECT_EQ(sum, 13); // 1+2+4+6=13

    sum = 0;
    it = fermat::ranges::for_each_n(v1, sz, rfun);
    EXPECT_EQ(it, v1.end());
    EXPECT_EQ(sum, 13);
}
...
TEST(ForEachNTest, MemberFunctionPointer) {
    ...
    auto it = fermat::ranges::for_each_n(v2.begin(), 3, &S::p);
    EXPECT_EQ(it, v2.begin() + 3);
    EXPECT_EQ(sum, 7); // 1+2+4=7

    sum = 0;
    it = fermat::ranges::for_each_n(v2, 3, &S::p);
    EXPECT_EQ(it, v2.begin() + 3);
    EXPECT_EQ(sum, 7);
}
...
TEST(ForEachNTest, WithProjection) {
    ...
    auto it = fermat::ranges::for_each_n(v2.begin(), 4, fun, &S::i_);
    EXPECT_EQ(it, v2.begin() + 4);
    EXPECT_EQ(sum, 13);

    sum = 0;
    it = fermat::ranges::for_each_n(v2, 4, fun, &S::i_);
    EXPECT_EQ(it, v2.begin() + 4);
    EXPECT_EQ(sum, 13);
}
