...
constexpr bool test_constexpr() {
    ...
    bool ok = (res.in == ranges::end(input)) &&
              (res.out == ranges::end(tmp)) &&
              ranges::equal(tmp, IL{0, 1, 2, 3});
    return ok;
}
...
TEST(CopyTest, DelimitView) {
    ...
    auto res = ranges::copy(str, buf);
    *res.out = '\0';
    EXPECT_EQ(res.in, std::next(ranges::begin(str), static_cast<std::ptrdiff_t>(std::strlen(sz))));
...
TEST(CopyTest, DelimitViewMove) {
    ...
    char buf[50];
    auto str = delimit(sz, '\0');
    auto res = ranges::copy(std::move(str), buf);
    *res.out = '\0';
