...
TEST(InnerProductTest, BasicWithPointers) {
    ...
    auto r1 = fermat::ranges::inner_product(a, a + n, b, 0);
    EXPECT_EQ(r1, 56);
    auto r2 = fermat::ranges::inner_product(a, a + n, b, 10);
    EXPECT_EQ(r2, 66);

    /// Four-iterator version (both ranges fully specified)
    auto r3 = fermat::ranges::inner_product(a, a + n, b, b + n, 0);
    EXPECT_EQ(r3, 56);
    auto r4 = fermat::ranges::inner_product(a, a + n, b, b + n, 10);
    EXPECT_EQ(r4, 66);

    /// With custom binary operations: multiply and plus
    auto r5 = fermat::ranges::inner_product(a, a + n, b, b + n, 1,
                                    std::multiplies<int>(), std::plus<int>());
    /// Expected value: 1 * (1+6) * (2+5) * (3+4) * (4+3) * (5+2) * (6+1) = 7^6 = 117649
    EXPECT_EQ(r5, 117649);

    /// Array overload (if Fermat provides it)
    auto r6 = fermat::ranges::inner_product(a, b, 0);
    EXPECT_EQ(r6, 56);
    auto r7 = fermat::ranges::inner_product(a, b, 10);
    EXPECT_EQ(r7, 66);
}
...
TEST(InnerProductTest, WithProjection) {
    ...
    auto r = fermat::ranges::inner_product(a, a + n, b, b + n, 1,
                                   std::multiplies<int>(), std::plus<int>(),
                                   &S::i, &S::i);
    EXPECT_EQ(r, 117649);
}
