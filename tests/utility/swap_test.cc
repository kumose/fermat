...
#include <fermat/utility/swap.h>           /// ranges::swap, ranges::iter_swap
#include <fermat/view/zip.h>         /// ranges::views::zip
...
TEST(SwapTest, SwappableWithConcepts) {
    // Compile-time checks
    static_assert(!ranges::swappable_with<std::pair<int,int>&&, std::pair<int,int>&&>, "");
    static_assert(ranges::swappable_with<std::pair<int&,int&>&&, std::pair<int&,int&>&&>, "");
}

/// The following three tests check ranges::swap with tuple/pair proxies.
...
TEST(SwapTest, SwapTie) {
    // Compile‑time only: ranges::swap with std::tie should be well‑formed.
    int a = 0, b = 42;
    int c = 24, d = 82;
...
TEST(SwapTest, SwapPairsOfTupleProxies) {
    ...
    int e = 1, f = 2, g = 3, h = 4;
    ranges::swap(std::make_pair(std::tie(a, b), std::tie(c, d)),
...
TEST(SwapTest, SwapTuplesOfPairProxies) {
    int a = 0, b = 42, c = 24, d = 82;
    int e = 1, f = 2, g = 3, h = 4;
    ranges::swap(std::make_tuple(std::make_pair(std::ref(a), std::ref(b)),
