/// \file meta_gtest.cpp
/// Google Test conversion of Meta library test suite.
...
#include <fermat/meta/meta.h>   /// Meta library from Fermat

using namespace meta;

// ------------------------------------------------------------
// Forward declaration for lambda_test (used in pathological test)
// ------------------------------------------------------------
template<typename A, int B = 0>
struct lambda_test;

// ------------------------------------------------------------
// tuple_cat implementation (from original, to test meta utilities)
// ------------------------------------------------------------
namespace tc_detail {
...
// ------------------------------------------------------------
// Helper for checking invoke-ability (SFINAE)
// ------------------------------------------------------------
...
// ------------------------------------------------------------
// Compile-time static assertions (preserved from original)
// ------------------------------------------------------------

// tuple_cat test (compile-time)
...
static_assert(
    std::is_same<
        reverse<list<int[1], int[2], int[3], int[4], int[5], int[6], int[7], int[8], int[9], int[10],
...
struct fact2 : let<lazy::if_c<(N > 0), lazy::multiplies<meta::size_t<N>, defer_i<std::size_t, fact2, N - 1> >,
            meta::size_t<1> > > {
};
...
template<typename N>
struct factorial : let<if_c<N::value == 0, meta::size_t<1>, lazy::multiplies<N, factorial<lazy::dec<N> > > > > {
};
...
// filter
static_assert(std::is_same<meta::filter<list<int, double, short, float, long, char>, quote<std::is_integral> >,
                  list<int, short, long, char> >::value, "");
static_assert(std::is_same<meta::filter<list<int, double, short, float, long, char>, quote<std::is_floating_point> >,
                  list<double, float> >::value, "");

// find_index
using l1 = list<int, long, short, int>;
static_assert(meta::find_index<l1, int>{} == 0, "");
static_assert(meta::find_index<l1, long>{} == 1, "");
static_assert(meta::find_index<l1, short>{} == 2, "");
static_assert(meta::find_index<l1, double>{} == meta::npos{}, "");
static_assert(meta::find_index<l1, float>{} == meta::npos{}, "");
static_assert(meta::find_index<list<>, double>{} == meta::npos{}, "");
using lambda_find = meta::lambda<_a, _b, meta::lazy::find_index<_b, _a> >;
static_assert(meta::invoke<lambda_find, long, l1>{} == 1, "");

// reverse_find_index
static_assert(meta::reverse_find_index<l1, int>{} == 3, "");
static_assert(meta::reverse_find_index<l1, long>{} == 1, "");
static_assert(meta::reverse_find_index<l1, short>{} == 2, "");
static_assert(meta::reverse_find_index<l1, double>{} == meta::npos{}, "");
static_assert(meta::reverse_find_index<l1, float>{} == meta::npos{}, "");
static_assert(meta::reverse_find_index<list<>, double>{} == meta::npos{}, "");
using lambda_rfind = meta::lambda<_a, _b, meta::lazy::reverse_find_index<_b, _a> >;
static_assert(meta::invoke<lambda_rfind, long, l1>{} == 1, "");

// count, count_if
static_assert(meta::count<l1, int>{} == 2, "");
static_assert(meta::count<l1, short>{} == 1, "");
static_assert(meta::count<l1, double>{} == 0, "");
static_assert(meta::count_if<l1, lambda<_a, std::is_same<_a, int> > >{} == 2, "");
static_assert(meta::count_if<l1, lambda<_b, std::is_same<_b, short> > >{} == 1, "");
static_assert(meta::count_if<l1, lambda<_c, std::is_same<_c, double> > >{} == 0, "");

// pathological lambda test (requires lambda_test declared but not defined)
using X = invoke<lambda<_a, lambda_test<_a> >, int>;
static_assert(std::is_same<X, lambda_test<_a> >::value, "");

// unique
static_assert(std::is_same<meta::unique<list<int, short, int, double, short, double, double> >,
                  list<int, short, double> >::value, "");
...
static_assert(std::is_same<invoke<lambda<_args, list<_args> >, int, short, double>,
                  list<int, short, double> >::value, "");
static_assert(std::is_same<invoke<lambda<_a, lambda_test<_a> >, int>,
                  lambda_test<_a> >::value, "");
static_assert(std::is_same<invoke<lambda<_args, defer<std::pair, _args> >, int, short>,
                  std::pair<int, short> >::value, "");
static_assert(std::is_same<invoke<lambda<_args, list<_args, list<_args> > >, int, short>,
                  list<int, short, list<int, short> > >::value, "");
static_assert(std::is_same<invoke<lambda<_a, _args, list<int, _args, double, _a> >, short *, short, float>,
...
using A = invoke<lambda<_a, lazy::invoke<lambda<_b, _args, list<_args, _b> >, _a,
    ...
    lazy::_t<std::add_lvalue_reference<_a> > > >, int>;
static_assert(std::is_same<A, list<int *, int &, int> >::value, "");
using B = invoke<lambda<_a, _args, lazy::invoke<lambda<_b, list<_b, _args, _a> >, _a> >, int, short, double>;
static_assert(std::is_same<B, list<int, short, double, int> >::value, "");
using ZZ = invoke<lambda<_a, _args_a,
    lazy::invoke<lambda<_b, _args_b, list<_b, _a, list<_args_b>, list<_args_a> > >,
        _args_a> >, int, short, float, double>;
...
// _z user-defined literal
static_assert(42_z == 42, "");

// integer_range
static_assert(
    meta::fold<meta::as_list<meta::integer_range<std::size_t, 0, 5> >, meta::size_t<0>, meta::quote<meta::plus> >{} ==
    10, "");
static_assert(
    meta::fold<meta::as_list<meta::integer_range<std::size_t, 5, 10> >, meta::size_t<0>, meta::quote<meta::plus> >{} ==
    35, "");
static_assert(
    meta::fold<meta::as_list<meta::integer_range<std::size_t, 0, 20> >, meta::size_t<0>, meta::quote<meta::plus> >{} ==
    190, "");
static_assert(
    std::is_same<meta::integer_range<std::size_t, 5, 10>, meta::integer_sequence<std::size_t, 5, 6, 7, 8, 9> >::value,
    "");

// reverse_fold
static_assert(
    meta::reverse_fold<meta::as_list<meta::integer_range<std::size_t, 0, 5> >, meta::size_t<0>, meta::quote<
        meta::plus> >{} == 10, "");
static_assert(
    meta::reverse_fold<meta::as_list<meta::integer_range<std::size_t, 5, 10> >, meta::size_t<0>, meta::quote<
        meta::plus> >{} == 35, "");
static_assert(
    meta::reverse_fold<meta::as_list<meta::integer_range<std::size_t, 0, 20> >, meta::size_t<0>, meta::quote<
        meta::plus> >{} == 190, "");

// ------------------------------------------------------------
// Runtime test for tuple_cat
// ------------------------------------------------------------
TEST(MetaTest, TupleCat) {
